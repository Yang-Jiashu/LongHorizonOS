...
def run_trial_ids(
    ...
    *,
    ...
    # Register the exact bytes first and use the authority-issued observation
    # token for D3.  The old integer-only repair API is retained for
    # compatibility, but benchmark code should exercise the proof-carrying
    # path so results do not depend on synthetic ``body-vN`` facts.
    versions[artifact_id] = 2
    invalidation_started = time.perf_counter()
    observation = runtime.observe_artifact(
        goal,
        artifact_id,
        2,
        f"{artifact_id}-v2",
    )
    repair = runtime.repair(goal, observation=observation)
