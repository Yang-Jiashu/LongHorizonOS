...
def resolved_base_url(explicit: str | None = None) -> str:
    """Base URL for the live probe, overridable without editing code.

    The client speaks the standard OpenAI chat-completions shape, so any
    compatible gateway works by pointing these two settings at it -- for example
    ``LHOS_LIVE_BASE_URL=https://api.deepseek.com`` with
    ``LHOS_LIVE_MODEL=deepseek-chat``.  Kept as environment settings so a caller
    never has to put a key or an endpoint into a tracked file.
    """

    return explicit or os.environ.get("LHOS_LIVE_BASE_URL", "").strip() or DEFAULT_BASE_URL


def resolved_model(explicit: str | None = None) -> str:
    """Model id for the live probe, overridable without editing code."""

    return explicit or os.environ.get("LHOS_LIVE_MODEL", "").strip() or DEFAULT_MODEL
