...
def run_recovery_repair(
    *, workspace_dir: str | None = None, state_path: str | None = None, pause: float = 0.0
    ...
    # Repair from an authority-issued observation rather than the deprecated
    # integer-only compatibility API.  ``apply_workspace_mutation`` already
    # registered the exact bytes in FactsProvider, so no synthetic content is
    # needed here.
    observation = os_.observe_artifact(goal, "source.py", 2)
    rep = os_.repair(goal, observation=observation)
