...
class SQLiteStorage:
    ...
    def _init_schema(self) -> None:
        with self.transaction():
            ...
            if "side_effect_class" not in action_columns:
                self.conn.execute(
                    "ALTER TABLE actions_projection "
                    "ADD COLUMN side_effect_class TEXT NOT NULL DEFAULT 'pure'"
                )
            if "recovery_policy" not in action_columns:
                self.conn.execute(
                    "ALTER TABLE actions_projection "
                    "ADD COLUMN recovery_policy TEXT NOT NULL DEFAULT 'retry'"
                )
            if "retry_count" not in action_columns:
                self.conn.execute(
                    "ALTER TABLE actions_projection "
