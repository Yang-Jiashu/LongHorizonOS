...
class SyscallDispatcher:
    ...
    async def _handle_submit_action(self, req: SubmitActionRequest) -> KernelEvent:
        if self._strict_effect_contracts:
            missing_fields = [
                field
                for field in ("side_effect_class", "recovery_policy")
                if field not in req.model_fields_set
            ]
            if missing_fields:
                # Reject before capability checks, Action projection, or lease
                # acquisition.  The defaults on SubmitActionRequest remain a
                # compatibility feature, but strict callers must make the
                # effect contract explicit at the syscall boundary.
                ev = KernelEvent(
                    pid=req.pid,
        ...
        acb = self._action_service.submit(
            ...
            recovery_policy=req.recovery_policy,
    ...
    async def _handle_exit(self, req: ExitRequest) -> KernelEvent:
        # Close lease admission before cleanup. LeaseService checks process
        # state under the same SQLite writer lock used for insertion, so a
        # racing acquire either lands first and is removed below, or observes
        # EXITED and is rejected.
