...
class RecoveryPolicy(StrEnum):
    """Recovery strategy for an action whose external outcome is uncertain.

    ``RETRY`` preserves the historical behaviour, but the Kernel still applies
    the side-effect safety rules (pure actions may retry; idempotent actions
    are inspected; irreversible/unknown actions become ``UNCERTAIN``).
    ``INSPECT`` forbids a blind redispatch and asks the driver for durable
    status instead.  ``UNCERTAIN`` is an explicit fail-closed policy useful
    for sinks without an inspection API.  ``DEAD_LETTER`` is represented by a
    terminal ``UNCERTAIN`` action with a dead-letter diagnostic because the
    Action state machine intentionally has no separate dead-letter state yet.
    """
