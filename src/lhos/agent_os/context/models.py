...
class ContentRef(BaseModel):
    ...
    # Recoverability signal for the recoverability_residual_lifecycle_v1 policy
    # (CWL, arXiv 2606.11213). True when this ref's content is already durably
    # persisted / re-derivable elsewhere (its effects are captured), so it is
    # the cheapest thing to omit under budget pressure — it can be re-derived.
    # Defaults False (conservative: assume not recoverable). Deliberately NOT
    # part of ref_hash()/manifest_hash(): it is selection-policy input metadata,
    # not binding identity, and folding it in would change every existing
    # manifest_hash and thus corrupt the meaning of durable priority_stable_v1
    # snapshots. This mirrors how `metadata` is already excluded from the hash.
