...
class _VersionContentCache:
    """One load's byte cache for exact ArtifactVersions.

    An ``(artifact_id, version)`` pair names immutable committed bytes -- that is
    the whole point of version binding, and ``_resolve_and_verify_ref`` checks
    the declared content hash before anything else is allowed to use them. So
    reading the same version twice within a single load cannot return different
    bytes, and caching them changes no hash and no selection decision.

    Without it each ref was read in full ``2 + selected_pages`` times: once to
    verify and then discard, once to paginate, and once more per selected page
    purely to slice a range out of it. For an artifact of size ``B`` split into
    ``P`` pages that is ``O(P*B)`` of file I/O to materialize ``B`` bytes.
    """

    def __init__(self, inner: VersionContentProvider) -> None:
        self._inner = inner
        self._bytes: dict[tuple[str, int, str], bytes] = {}
        self.underlying_reads = 0
        self.served_reads = 0

    def read_version(self, *, artifact_id: str, version: int, canonical_uri: str) -> bytes:
        self.served_reads += 1
        key = (artifact_id, version, canonical_uri)
        cached = self._bytes.get(key)
        if cached is None:
            self.underlying_reads += 1
            cached = self._inner.read_version(
...
class ContextService:
    ...
    def _sink_page(
        ...
        *,
        ...
        content: VersionContentProvider | None = None,
    ) -> list[ContextPage]:
        return compute_pages_for_ref(
            ref=ref,
            content_supplier=content if content is not None else self._content,
    ...
    def _resolve_and_verify_ref(
        self,
        ref: ContentRef,
        *,
        caller_pid: str,
        content: VersionContentProvider,
    ) -> bytes:
        ...
        try:
            content_bytes = content.read_version(
                artifact_id=ref.artifact_id,
                version=ref.version,
        ...
        actual_hash = _content_hash_for(content_bytes)
    ...
    def _pages_for_manifest(
        ...
        *,
        ...
        for ref in sorted_refs:
            self._resolve_and_verify_ref(ref, caller_pid=caller_pid, content=content)
            pages = self._sink_page(ref=ref, manifest=manifest, content=content)
    ...
    def _materialize(
        ...
        *,
        ...
        # Pages of one artifact all resolve their media type from the same ref;
        # scanning ``manifest.refs`` per page made this O(pages * refs).
        ref_by_artifact = {ref.artifact_id: ref for ref in manifest.refs}
        loaded_pages: list[LoadedPage] = []
        bindings: list[VersionBinding] = []
        for page in selected:
            byte_start = page.byte_start
            byte_end = page.byte_end
            content_bytes = source.read_version(
                artifact_id=page.artifact_id,
                version=page.version,
                canonical_uri=page.canonical_uri,
            )[byte_start:byte_end]
            # recover media/encoding from manifest ref
            ref = ref_by_artifact.get(page.artifact_id)
    ...
    def load(
        ...
        *,
        ...
        # One cache for the whole load: verification, pagination and
        # materialization all read the same immutable ArtifactVersions.
        content = _VersionContentCache(self._content)

        # resolve + paginate
        ref_pages_list = self._pages_for_manifest(manifest, caller_pid=caller_pid, content=content)

        # select under the manifest's declared policy. priority_stable_v1
        # dispatches verbatim; recoverability_residual_lifecycle_v1 additionally
        # honours a declared read-set sourced from manifest metadata.
        selected, omitted_ids, tokens_used, bytes_used = select_pages(
            manifest=manifest,
            ref_pages=ref_pages_list,
            read_set=read_set_from_metadata(manifest),
        )
