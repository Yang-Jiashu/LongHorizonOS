...
class JournalService:
    ...
    def append_events_atomically(self, events: list[KernelEvent]) -> list[KernelEvent]:
        ...
        # Offset allocation is a read-modify-write sequence shared by every
        # connection to the journal file.  Take SQLite's writer reservation
        # before reading next_offset so two processes cannot both allocate the
        # same value or fail while upgrading concurrent deferred transactions.
    ...
    def read_last_for_pid_since(
        self, pid: str, offset: int
    ) -> KernelEvent | None:
        """Newest event for ``pid`` at or after ``offset``, in O(log n).

        Served by ``idx_journal_pid_offset``.  This replaces the kernel's
        "read the first 100 events past the cursor, then filter by pid"
        pattern: that window could skip a process's own event entirely once
        more than 100 events interleaved between two of its steps, while this
        indexed DESC/LIMIT-1 lookup is exact regardless of interleaving.
        """

        row = self._storage.query_one(
            """SELECT * FROM journal_events
               WHERE pid = ? AND journal_offset >= ?
               ORDER BY journal_offset DESC
               LIMIT 1""",
            (pid, offset),
        )
    ...
    def rebuild_projections(self, handlers: list[Any]) -> int:
        ...
        with self._storage.transaction() as tx:
            ...
            tx.execute(
                ...
                "(SELECT COALESCE(MAX(journal_offset) + 1, 0) FROM journal_events))"
            )
