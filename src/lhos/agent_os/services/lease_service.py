...
class LeaseService:
    ...
    def atomic_acquire(
        ...
        with self._storage.transaction(immediate=True) as tx:
            # Terminal processes must not start a new ownership epoch.  Keep
            # this check in the same writer transaction as lease insertion:
            # lifecycle cleanup marks the process terminal first, so a racing
            # acquire either linearizes before that transition (and is then
            # removed by cleanup) or observes the terminal state and fails.
            #
            # Unknown pids remain allowed for the low-level LeaseService API;
            # recovery/audit callers intentionally use it without a PCB.
            process_row = tx.query_one(
                "SELECT state FROM processes_projection WHERE pid = ?",
                (pid,),
            )
            if process_row is not None:
                try:
                    process_state = ProcessState(process_row["state"])
                except ValueError as exc:
                    raise KernelInvariantViolation(
                        f"process {pid!r} has invalid state {process_row['state']!r}"
                    ) from exc
                if process_state in TERMINAL_PROCESS_STATES:
                    raise LeaseAcquisitionFailed(pid, claims[0]["resource_id"])
    ...
    def release(self, lease_ids: list[str]) -> int:
        ...
        with self._storage.transaction(immediate=True) as tx:
            ...
            # Lease deletion and its audit records must linearize together.
            # Keeping journal publication inside the same writer transaction
            # prevents a crash window where the projection says RELEASED but
            # replay has no LEASE_RELEASED event.
            self._journal.append_events_tx(tx, events)
        return released

    def release_all_for_pid(self, pid: str) -> int:
        """Release every lease and wait edge owned by ``pid`` atomically.

        The previous implementation selected lease ids in one transaction,
        cleared waiters in a second transaction, and finally delegated to
        :meth:`release` in a third transaction.  That left a TOCTOU window:
        another thread could acquire a lease for the same pid after the
        initial SELECT, leaving it orphaned, or a journal failure could leave
        the projection and waiter table only partially cleaned up.

        Process cleanup is a single lifecycle transition, so selection,
        deletion, waiter cleanup, and ``LEASE_RELEASED`` journal publication
        now share one ``BEGIN IMMEDIATE`` transaction.  A journal failure
        therefore rolls back *all* cleanup side effects and callers receive
        the original exception instead of a misleading partial count.
        """
        ...
        with self._storage.transaction(immediate=True) as tx:
            ...
            for row in rows:
                deleted = tx.execute(
                    "DELETE FROM leases_projection WHERE lease_id = ? AND owner_pid = ?",
                    (row["lease_id"], pid),
                )
                # The writer lock makes this predicate deterministic, but keep
                # the row-count guard so a future schema/trigger cannot cause
                # an audit event for a row that was not actually removed.
            ...
            # Wait edges are part of the process-owned resource state.  Clear
            # them in the same transaction as lease deletion so a crash cannot
            # expose a half-cleaned process.
            tx.execute("DELETE FROM lease_waiters WHERE pid = ?", (pid,))

            # Journal is authoritative; publish release events before commit.
            self._journal.append_events_tx(tx, events)
        return released
    ...
    def renew(self, lease_id: str, ttl: timedelta = DEFAULT_LEASE_TTL) -> ResourceLease | None:
        ...
        with self._storage.transaction(immediate=True) as tx:
            # Take the clock sample only after acquiring the writer lock.
            # Otherwise a renewal blocked behind another writer could compare
            # against a pre-wait timestamp and revive a lease that expired
            # while it was waiting to enter this transaction.
            check_now = datetime.now(UTC)
            new_expiry = check_now + ttl
            cur = tx.execute(
            ...
            # Renewal and its journal record must share one writer
            # transaction.  Otherwise a process crash after the projection
            # UPDATE but before append_event() leaves replay unaware of the
            # extended lease and violates the Journal-as-authority contract.
            self._journal.append_events_tx(
                tx,
                [
                    KernelEvent(
                        pid=row["owner_pid"],
    ...
    def _find_cycles(wait_for: dict[str, set[str]]) -> list[list[str]]:
        """Find wait-for cycles deterministically.

        Each cycle is rotated to start at its smallest pid, deduplicated, and the
        returned list is sorted, so the same wait-for graph always yields the
        same answer.  That matters because ``_recover_deadlock`` kills a victim
        chosen from ``cycles[0]``: the previous version iterated an unsorted
        ``set`` of neighbours, so on the graph
        ``{p1: {p2, p3}, p2: {p3, p1}, p3: {p1, p2}}`` four different
        ``PYTHONHASHSEED`` values produced four different results -- and
        therefore selected different victims for identical state.

        This is a depth-first search that reports the back edges it encounters.
        It is deliberately **not** a complete simple-cycle enumeration (that
        needs Johnson's algorithm); the previous implementation was not one
        either, despite its docstring.  Deadlock recovery needs at least one
        genuine cycle per deadlocked group, and every cycle reported here is
        genuine.

        The traversal is iterative so a deep wait chain cannot exhaust the
        interpreter's recursion limit.
        """

        adjacency = {node: sorted(targets) for node, targets in wait_for.items()}
        ...
        for root in sorted(adjacency):
            ...
            stack.append(root)
            stack_position[root] = 0
            while work:
                _node, neighbours = work[-1]
                descended = False
                for neighbour in neighbours:
                    position = stack_position.get(neighbour)
                    ...
                    if neighbour in visited:
                        continue
                    stack_position[neighbour] = len(stack)
