...
class ActionService:
    ...
    def reserve_retry_if_fenced(
        self,
        action_id: str,
        *,
        now: datetime | None = None,
    ) -> tuple[bool, str | None]:
        """Atomically consume the single retry budget under the current fence.

        The reservation is durable before the external redispatch. If the
        process crashes after this transaction, replay preserves the consumed
        budget and recovery can inspect the sink but cannot issue another
        blind retry.
        """
        ...
        with self._storage.transaction(immediate=True) as tx:
            ...
            if (
                acb.side_effect_class != SideEffectClass.PURE
                or acb.recovery_policy != RecoveryPolicy.RETRY
            ):
                return False, "retry_not_permitted"
            if acb.retry_count >= 1:
                return False, "retry_budget_exhausted"

            fence_error = self._validate_fencing_contract_tx(
                tx,
                acb,
                reservation_time,
            )
            ...
            ev = KernelEvent(
                pid=acb.pid,
                event_type="ACTION_RETRY_RESERVED",
                payload={
                    "action_id": action_id,
                    "retry_count": acb.retry_count,
                    "fencing_tokens": acb.fencing_tokens,
                },
    ...
    def validate_reserved_retry_if_fenced(
        self,
        action_id: str,
        *,
        now: datetime | None = None,
    ) -> tuple[bool, str | None]:
        """Revalidate a consumed retry immediately before external dispatch.

        Reservation and driver dispatch cannot share one database transaction
        because the latter is an external call. This second check closes
        cancellation and lease-supersession races that occur after the durable
        retry reservation but before the Kernel invokes the driver.
        """
        ...
        with self._storage.transaction(immediate=True) as tx:
            ...
            if (
                ...
                return False, "retry_not_permitted"
            if acb.retry_count != 1:
                return False, "retry_not_reserved"
            fence_error = self._validate_fencing_contract_tx(
                tx,
                acb,
                validation_time,
    ...
    def handle_event(self, ev: KernelEvent) -> None:
        if ev.event_type == "ACTION_SUBMITTED":
            ...
            # ACTION_SUBMITTED rows written before durable retry budgeting did
            # not record whether a blind retry had already happened. Rebuild
            # them conservatively with an exhausted budget; new writers always
            # include retry_count=0 in the event payload.
            payload.setdefault("retry_count", 1)
            acb = ActionControlBlock(**payload)
            self._upsert_projection(acb)
        elif ev.event_type in (
            "ACTION_ADMITTED",
            "ACTION_INTENT_DURABLE",
            "ACTION_RUNNING",
            "ACTION_RETRY_RESERVED",
            "ACTION_COMMITTED",
            "ACTION_FAILED",
            "ACTION_TIMED_OUT",
            "ACTION_UNCERTAIN",
            ...
            if ev.event_type == "ACTION_RETRY_RESERVED":
                try:
                    retry_count = int(ev.payload.get("retry_count", 1))
                except (TypeError, ValueError):
                    retry_count = 1
                current_acb.retry_count = max(current_acb.retry_count, retry_count, 1)
        ...
        tx.execute(
            """INSERT INTO actions_projection
               (action_id, pid, device_type, operation, arguments_json, state,
                ...
                idempotency_key, side_effect_class, recovery_policy, retry_count,
    ...
    def _row_to_acb(row: dict[str, Any]) -> ActionControlBlock:
        # Durable projection values may have been written by an older binary,
        # a partially applied migration, or a corrupted operator edit.
        # Recovery classification is safety-critical: never let an invalid
        # value raise during startup/query and accidentally bypass recovery.
        # Normalize malformed values to the strictest fail-closed classes.
        try:
            side_effect_class = SideEffectClass(row.get("side_effect_class", "pure"))
        except (TypeError, ValueError):
            side_effect_class = SideEffectClass.UNKNOWN
        try:
            recovery_policy = RecoveryPolicy(row.get("recovery_policy", RecoveryPolicy.RETRY.value))
        except (TypeError, ValueError):
            recovery_policy = RecoveryPolicy.UNCERTAIN
        try:
            retry_count = int(row.get("retry_count", 1))
            if retry_count < 0:
                raise ValueError("retry_count must be non-negative")
        except (TypeError, ValueError, OverflowError):
            # A malformed or pre-budget row must never mint a new retry.
            retry_count = 1
