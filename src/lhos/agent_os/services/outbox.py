...
class TransactionalOutbox:
    ...
    async def dispatch_once(
        ...
        *,
        ...
        # ``now`` is often supplied by deterministic recovery tests.  It is
        # still the *start* of the delivery attempt, not an authority that
        # freezes time while an arbitrary publisher is running.  Track
        # monotonic elapsed time so an explicit logical timestamp remains
        # reproducible while a slow publisher can no longer acknowledge an
        # expired claim with the stale start timestamp.
        ...
        try:
            ...
            completion_time = dispatch_time + timedelta(
                seconds=max(0.0, time.monotonic() - started_monotonic)
            )
            terminal = max_attempts is not None and record.attempts >= max_attempts
            # Keep a zero-delay retry anchored to the caller's logical
            # dispatch timestamp rather than the wall-clock duration of the
            # publisher call.  ``completion_time`` is intentionally used for
            # fencing (``nack`` must reject a claim that expired while the
            # publisher was running), but adding that elapsed duration to
            # ``retry_at`` makes a zero-delay retry unexpectedly unavailable
            # when a busy process takes a few microseconds between attempts.
            # In particular, deterministic callers that pass an explicit
            # ``now`` expect ``retry_delay=0`` to mean immediately eligible
            # at that same logical timestamp.  Positive delays remain relative
            # to completion, preserving the usual backoff semantics.
            retry_at = (
                dispatch_time if retry_delay == timedelta(0) else completion_time + retry_delay
            )
            self.nack(
                record.outbox_id,
                record.claim_token,
    ...
    async def dispatch_ready(
        ...
        *,
        ...
        for _ in range(limit):
            ...
            # Advance a deterministic scan clock by the actual delivery
            # duration.  This prevents a slow publisher from making the
            # remainder of a drain pass reason about an earlier timestamp.
            cursor_time += timedelta(seconds=max(0.0, time.monotonic() - iteration_started))
