...
def propagate_invalidation(
    ...
    while queue:
        ...
        # The graph structure (edges) is not mutated by the set_state calls
        # above, only node states are — so the graph loaded once stays valid
        # for the whole propagation. Avoids reloading the full graph from the
        # store for every propagated node (O(K*(V+E)) -> O(V+E+K)).
...
def invalidation_metrics(event_store, run_id: str) -> list[dict]:
    ...
    # Index starts by node so the per-propagation re-executed lookup is
    # O(affected) instead of O(all starts) — deterministic, same result.
    starts_by_node: dict[str, list] = {}
    for e in starts:
        starts_by_node.setdefault(e.payload.get("node_id"), []).append(e)
    report: list[dict] = []
    for prop in propagations:
        affected = set(prop.payload.get("affected_node_ids", []))
        re_executed = set()
        for node_id in affected:
            for e in starts_by_node.get(node_id, ()):
                if (
                    e.sequence > prop.sequence
                    and e.payload.get("retry_reason") in {"stale", "invalidated"}
                ):
                    re_executed.add(node_id)
                    break
        report.append(
            {
