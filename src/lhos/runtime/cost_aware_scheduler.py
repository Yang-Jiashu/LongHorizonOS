...
class CostAwareScheduler:
    ...
    def score(
        ...
        now: datetime,
        criticality_map: dict[str, float] | None = None,
        ...
        # Batch DP maps: build once per scheduling round and query each node
        # in O(1) instead of re-walking the DAG per candidate.
        if criticality_map is None:
            criticality_map = critical_path.criticality_profile(graph)
        if unlock_map is None:
            unlock_map = critical_path.unlock_profile(graph)
        return (
            w["criticality"] * criticality_map.get(node.id, 0.0)
            + w["unlock"] * unlock_map.get(node.id, 0.0)
