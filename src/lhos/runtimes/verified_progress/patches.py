...
class AddNodeOp(BaseModel):
    ...
    evidence_hash: str = ""
    produced_by_pid: str = ""
    # Causal binding to the scheduler ownership epoch.  These are optional
    # for backwards-compatible pure VPG patches; secure SDK paths must set
    # all three before semantic completion is accepted.
    claim_id: str | None = None
    attempt_id: str | None = None
    semantic_epoch: int | None = None
    # Exact Kernel lease generation which authorized this Evidence commit.
    # Legacy graph authors may omit it; fenced SDK commits populate it.
    lease_fencing_token: int | None = None
    # Canonical SHA-256 digest of the provenance/coverage report associated
    # with this Evidence.  Optional for legacy pure-VPG patches.
    provenance_digest: str | None = None
    # Exact Context VM materialization used by the producing attempt.
