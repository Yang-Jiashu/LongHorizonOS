...
from .readiness import compute_ready_frontier, compute_ready_task_ids
from .verification import build_verification_indices, task_is_verified
...
class VerifiedProgressRuntime:
    ...
    def submit_patch(
        ...
        *,
        ...
        _allow_large_operations: bool = False,
        _allow_repair_frontier: bool = False,
        _invalidation_seed_task_ids: tuple[str, ...] = (),
        _commit_guard: LeaseCommitGuard | None = None,
        _d3_record: dict[str, Any] | None = None,
        _read_guards: tuple[Any, ...] | None = None,
    ) -> PatchCommitResult:
        ...
        # Idempotency replay bypasses optimistic graph-version validation, but
        # it must not bypass ownership or cognition freshness.  Resolve the
        # durable row and validate both guards under one GraphStore writer
        # transaction, so a stale worker cannot replay an old result after an
        # input Artifact has advanced.
        idem = self.store.idempotent_replay(
            patch.composite_key,
            commit_guard=_commit_guard,
            read_guards=_read_guards,
        )
        if idem is not None:
        ...
        req.allow_large_operations = _allow_large_operations
        ...
        derived_events = self._apply_derived_after_patch(
            ...
            invalidation_seed_task_ids=_invalidation_seed_task_ids,
        )
        all_events = list(result.events) + derived_events

        # Upsert every node whose exact canonical representation changed.
        # History hashes bind this representation, including dict key order,
        # so semantic model equality is intentionally insufficient here.
        nodes_to_upsert = []
        ...
        try:
            ...
            # re-raised unchanged.
            raced_idem = self.store.idempotent_replay(
                patch.composite_key,
                commit_guard=_commit_guard,
                read_guards=_read_guards,
            )
    ...
    def refresh_derived_state(
        ...
        *,
        ...
        invalidation_seed_task_ids: tuple[str, ...] = (),
        read_guards: tuple[Any, ...] = (),
    ) -> PatchCommitResult:
        ...
        """
        current = (
            self.get_graph(graph_id).current_version
            if expected_graph_version is None
            else int(expected_graph_version)
        )
        return self.submit_patch(
            ...
            _invalidation_seed_task_ids=tuple(invalidation_seed_task_ids),
            _read_guards=tuple(read_guards),
            _d3_record=d3_record,
        )
    ...
    def _apply_derived_after_patch(
        ...
        *,
        ...
        return _recompute_derived_state(
            ...
            invalidation_seed_task_ids=invalidation_seed_task_ids,
        )
    ...
    def get_recent_events(
        ...
        *,
        ...
        """Return the bounded durable event tail for one pinned graph view."""
    ...
    def get_d3_results(
        ...
        """Return durable D3 invalidation envelopes for one graph.

        The GraphStore owns persistence and integrity validation; this
        read-only delegate keeps the query available through the same runtime
        object used for graph commits.  Unknown graphs fail consistently with
        the other SDK inspection methods instead of silently returning an
        empty list.
        """

        self.get_graph(graph_id)
        return self.store.get_d3_results(
            graph_id,
...
def _recompute_derived_state(
    ...
    *,
    ...
    # Edge adjacency for the Task VERIFIED predicate, built once for the whole
    # pass.  ``task_is_verified`` is evaluated for every Task below; without a
    # shared index each call re-scanned the entire edge list, making a single
    # commit O(tasks * edges) (super-linear in graph size and the dominant cost
    # when compiling a large Goal).  Edges are immutable during derivation, so
    # one build is safe and changes no predicate or ordering.
    verifies_index, produces_index = build_verification_indices(edges)
    # Same reasoning, for the two predicates below that walked the edge list
    # once per Task.  Built here rather than in ``build_verification_indices``
    # so that helper's contract stays exactly what its name says.
    depends_index: dict[str, list[str]] = {}
    for edge in edges:
        if edge.edge_type == EdgeType.DEPENDS_ON:
            depends_index.setdefault(edge.source_node_id, []).append(edge.target_node_id)

    # 1. explicit external-input invalidation.  These seeds are admitted only
    # by trusted composition-root reconciliation code after it validates the
    # exact authority observation and declared task/resource relationship.
    # Keeping the mutation inside the normal candidate projection makes the
    # seed, transitive propagation, Goal reopening, ready frontier, and D3
    # envelope part of the same GraphStore transaction.
    for task_id in sorted(set(invalidation_seed_task_ids)):
        task = nodes.get(task_id)
        if not isinstance(task, TaskNode) or task.validity != NodeValidity.VERIFIED:
            continue
        _mark_task_stale(
            task,
            graph_id=graph_id,
    ...
    # 2. task-local invalidation
    for n in list(nodes.values()):
        if not isinstance(n, TaskNode):
            continue
        if n.validity != NodeValidity.VERIFIED:
            continue
        pinned_now = _pinned_versions(n.node_id, nodes, produces_index)
        ...
        if (
            ...
            or _has_unverified_dependency(n.node_id, nodes, depends_index)
        ):
            _mark_task_stale(
                n,
                graph_id=graph_id,
    ...
    while changed:
        ...
        for n in list(nodes.values()):
            if not isinstance(n, TaskNode) or n.validity != NodeValidity.VERIFIED:
                continue
            if not _has_unverified_dependency(n.node_id, nodes, depends_index):
                continue
            _mark_task_stale(
                n,
                graph_id=graph_id,
    ...
    # 3. VERIFIED/STALE for all tasks
    for n in list(nodes.values()):
        if not isinstance(n, TaskNode):
            continue
        if n.validity == NodeValidity.VERIFIED:
            ...
            n.metadata["__verified_artifact_versions"] = [
                ...
                for u, v in _pinned_versions(n.node_id, nodes, produces_index)
            ]
            continue
        if n.validity == NodeValidity.STALE:
            ...
            if _has_superseded_artifact(
                _pinned_versions(n.node_id, nodes, produces_index),
                facts_artifact,
            ):
                continue
            ...
            if task_is_verified(
                ...
                verifies_index=verifies_index,
                produces_index=produces_index,
            ):
    ...
    # 5. ready frontier
    frontier_ids = tuple(compute_ready_task_ids(nodes, edges))
    out.append(
        GraphEvent(
            graph_id=graph_id,
...
def _pinned_versions(
    ...
    Takes the PRODUCES adjacency rather than the edge list: this is evaluated
    several times per Task inside one derivation pass, and re-walking every edge
    each time made a single commit O(tasks * edges).  Selecting the maximum
    version per uri is order-independent, so using the adjacency changes no
    result.
    """
    latest: dict[str, tuple[str, int]] = {}
    for target_node_id in produces_index.get(task_id, ()):
        n = nodes.get(target_node_id)
...
def _has_unverified_dependency(
    ...
    """Whether any declared dependency of the Task is not VERIFIED.

    Takes the DEPENDS_ON adjacency for the same reason as ``_pinned_versions``:
    scanning every edge once per Task made one derivation pass quadratic in
    graph size.
    """

    for target_node_id in depends_index.get(task_id, ()):
        dep = nodes.get(target_node_id)
...
def _ops_to_nodes_edges(graph_id: str, patch: GraphPatchProposal) -> tuple[list, list]:
    ...
    def _node_for(op: AddNodeOp) -> AnyNode | None:
        ...
        base: dict[str, object] = dict(
            ...
            semantic_epoch=op.semantic_epoch,
            lease_fencing_token=op.lease_fencing_token,
            provenance_digest=op.provenance_digest,
            context_snapshot_id=op.context_snapshot_id,
            context_manifest_id=op.context_manifest_id,
            context_manifest_hash=op.context_manifest_hash,
            context_working_set_hash=op.context_working_set_hash,
            context_materialized_hash=op.context_materialized_hash,
        )
