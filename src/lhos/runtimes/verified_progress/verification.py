...
def validate_evidence(
    ...
    *,
    ...
    # Provenance is an optional strengthening binding for SDK-produced
    # Evidence.  When the typed digest is present, it must agree with the
    # canonical coverage report persisted in the node metadata.  Do not
    # silently trust one copy while ignoring the other: a malformed or
    # edited report would otherwise be able to retain VERIFIED status after
    # the scheduler's causal checks have passed.  Legacy/core graph authors
    # may omit the digest entirely, in which case this additional check is
    # intentionally not applied.
    if evidence.provenance_digest is not None:
        provenance = evidence.metadata.get("provenance")
        report_hash = provenance.get("report_hash") if isinstance(provenance, dict) else None
        if (
            not isinstance(report_hash, str)
            or report_hash.strip().lower() != evidence.provenance_digest
        ):
            return _fail(
                VPGCode.EVIDENCE_PROVENANCE_DIGEST_MISMATCH,
                "evidence.provenance_digest does not match metadata provenance report_hash",
            )
...
def build_verification_indices(
    edges: list[VPGEdge],
) -> tuple[dict[str, list[str]], dict[str, list[str]]]:
    """Precompute edge adjacency for the Task VERIFIED predicate.

    ``verifies_index`` maps a Task id to the Verification node ids that verify
    it; ``produces_index`` maps a source node id to the target node ids it
    produces.  ``task_is_verified`` can then look these up in O(deg) instead of
    re-walking the whole edge list once per Task -- turning a per-Task O(E)
    probe into O(E) built once for the whole pass.  Purely a lookup structure:
    it changes no predicate and no ordering (adjacency lists keep edge order).
    """
    verifies_index: dict[str, list[str]] = {}
    produces_index: dict[str, list[str]] = {}
    for e in edges:
        if e.edge_type == EdgeType.VERIFIES:
            verifies_index.setdefault(e.target_node_id, []).append(e.source_node_id)
        elif e.edge_type == EdgeType.PRODUCES:
            produces_index.setdefault(e.source_node_id, []).append(e.target_node_id)
    return verifies_index, produces_index


def _find_verifications(
    task_id: str,
    nodes: dict,
    edges: list[VPGEdge],
    *,
    verifies_index: dict[str, list[str]] | None = None,
) -> list[VerificationNode]:
    out: list[VerificationNode] = []
    if verifies_index is not None:
        for source_id in verifies_index.get(task_id, ()):
            n = nodes.get(source_id)
...
def _find_evidence_for_verification(
    ...
    edges: list[VPGEdge],
    *,
    produces_index: dict[str, list[str]] | None = None,
) -> list[EvidenceNode]:
    out: list[EvidenceNode] = []
    if produces_index is not None:
        for target_id in produces_index.get(verification_id, ()):
...
def task_is_verified(
    ...
    *,
    ...
    ``verifies_index``/``produces_index`` are optional precomputed edge
    adjacency maps (see ``build_verification_indices``).  When supplied the
    Verification/Evidence lookups avoid a full O(E) edge scan per Task; when
    omitted the predicate falls back to the original scan and is unchanged.
    """
    ...
    verifications = _find_verifications(task.node_id, nodes, edges, verifies_index=verifies_index)
    valid_verification_count = 0
    for v in verifications:
        evidence_list = _find_evidence_for_verification(
            v.node_id, nodes, edges, produces_index=produces_index
        )
        has_valid = any(
            validate_evidence(
                ev,
