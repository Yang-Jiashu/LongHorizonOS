...
class EvidenceNode(VPGNode):
    ...
    produced_by_pid: str = ""
    # Optional causal ownership binding.  Legacy/core graph authors may omit
    # these fields, but the SDK's fenced execution path populates all three
    # and the scheduler uses them to prevent evidence from one worker/epoch
    # completing another worker's claim.
    claim_id: str | None = None
    attempt_id: str | None = None
    semantic_epoch: int | None = None
    # Exact Kernel lease generation that authorized the Evidence commit.
    # Optional for legacy/core graph authors; the built-in SDK path always
    # populates it and the Scheduler matches it against the active claim.
    lease_fencing_token: int | None = None
    # Digest of the canonical provenance/coverage report used by the SDK
    # verifier.  This is optional for legacy graph authors, but when present
    # it must be a SHA-256 digest and must agree with the report persisted in
    # ``metadata["provenance"]``.  Keeping the field on the typed Evidence
    # record makes the ownership/provenance binding queryable without parsing
    # opaque metadata.
    ...
    @classmethod
    def _provenance_digest_is_sha256(cls, value: str | None) -> str | None:
        if value is None:
            return None
        normalized = str(value).strip().lower()
        if len(normalized) != 64 or any(char not in "0123456789abcdef" for char in normalized):
            raise ValueError("provenance_digest must be a 64-character SHA-256 hex digest")
        return normalized
