...
class PatchValidationRequest:
    ...
    # Trusted composition-root compilation may submit one large proposal so
    # publication remains atomic.  Ordinary/agent-authored patches must keep
    # the MAX_PATCH_OPS DoS guard.
    allow_large_operations: bool = False
...
def validate_patch(req: PatchValidationRequest) -> PatchValidationResult:
    ...
    if len(patch.operations) > MAX_PATCH_OPS and not getattr(req, "allow_large_operations", False):
        raise VPGError(
    ...
    # Derived-state evaluation mutates top-level node fields and metadata only.
    # Isolate those from the decoded baseline without recursively copying the
    # entire graph. Existing edges are read-only; new edges are fresh objects.
    cand_nodes: dict[str, AnyNode] = {
        nid: node.model_copy(update={"metadata": dict(node.metadata)})
        for nid, node in req.current_nodes.items()
    }
    cand_edges: list[VPGEdge] = list(req.current_edges)
    # Edge-id uniqueness is checked for every AddEdge/Attach* operation.  A
    # repeated ``{e.edge_id for e in cand_edges}`` scan made a large patch
    # quadratic in the number of already-committed edges (and was especially
    # visible when compiling a long DAG).  Keep the same fail-closed check as
    # an O(1) membership test while retaining ``cand_edges`` as the ordered
    # projection source.
    cand_edge_ids: set[str] = {edge.edge_id for edge in cand_edges}
    ...
    for op in patch.operations:
        if isinstance(op, AddNodeOp):
            ...
            if op.edge_id in cand_edge_ids:
                raise VPGError(
            ...
            cand_edge_ids.add(new_edge.edge_id)
            ...
            cand_edge_ids.add(edge.edge_id)
