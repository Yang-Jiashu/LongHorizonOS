...
def _bind_claim_lease(claim: TaskClaim, lease: Any) -> None:
    ...
    Lease metadata comes from an untrusted provider boundary.  Reconciliation
    must still be able to release a lease when its fencing token is malformed:
    the token is required for *ownership admission*, but not for idempotent
    cleanup of an interrupted ACQUIRING claim.  Therefore malformed token data
    is represented as ``None`` and never allowed to abort the cleanup pass.
    """
    ...
    try:
        ...
        claim.lease_fencing_token = None
        prior_reason = claim.reason or ""
        claim.reason = (
            f"{prior_reason}; malformed_lease_fencing_token_ignored_for_cleanup"
            if prior_reason
            else "malformed_lease_fencing_token_ignored_for_cleanup"
        )
