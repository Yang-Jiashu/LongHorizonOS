...
class LeaseProvider(Protocol):
    ...
    def renew(self, lease_id: str, ttl: timedelta) -> LeaseInfo | None:
        """Extend one still-live lease without changing its fencing token.

        ``None`` means the authoritative lease no longer exists (or has
        already expired).  Providers must linearize renewal with release and
        reclamation; a stale owner must never be able to revive an old lease.
        """
