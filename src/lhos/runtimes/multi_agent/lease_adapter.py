...
class LeaseAdapter:
    ...
    def renew(self, lease_id: str, ttl: timedelta | None = None) -> Any | None:
        """Renew one claim lease through the injected authority.

        Renewal is intentionally a narrow pass-through: the provider owns the
        atomic check/update and fencing semantics.  A missing result means the
        lease was already released/expired and must be treated as lease loss.
        """
        requested_ttl = self._ttl if ttl is None else ttl
        if not isinstance(requested_ttl, timedelta) or requested_ttl <= timedelta(0):
            raise ValueError("lease renewal ttl must be a positive timedelta")
        renew = getattr(self._provider, "renew", None)
        if not callable(renew):
            # Fail closed for legacy providers which do not implement the
            # capability; callers must not assume an unrenewed lease remains
            # safe indefinitely.
            return None
        return renew(lease_id, requested_ttl)
