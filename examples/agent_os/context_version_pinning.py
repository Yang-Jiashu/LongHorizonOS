...
def main() -> None:
    ...
    v1 = next(v for v in sdk.list_versions("p1", "workspace:///doc.md")
              if v["version"] == 1)
    v2 = next(v for v in sdk.list_versions("p1", "workspace:///doc.md")
              if v["version"] == 2)
    ...
    manifest_v1 = ContextManifest(
        ...
        refs=(ContentRef(
            ...
            artifact_id=art_id, version=v1["version"],
            content_hash=v1["content_hash"], media_type="text/markdown",
    ...
    manifest_v2 = ContextManifest(
        ...
        refs=(ContentRef(
            ref_id="doc", canonical_uri="artifact://ns-p1/doc.md",
            artifact_id=art_id, version=v2["version"],
            content_hash=v2["content_hash"], media_type="text/markdown",
            priority=10, required=True,
        ),),
        token_budget=10_000,
        page_size_bytes=64,
    ...
    print(json.dumps({
        ...
        "v1_pages_decoded": [p.content.decode("utf-8", errors="replace")
                            for p in loaded_v1.ordered_pages],
        "v2_pages_decoded": [p.content.decode("utf-8", errors="replace")
                            for p in loaded_v2.ordered_pages],
