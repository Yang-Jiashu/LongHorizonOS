...
def cause(gid, ver, tid, aid="A"):
    return InvalidationCause(cause_id=f"c:{tid}", graph_id=gid, graph_version=ver,
                             ...
                             old_version=0, new_version=1, reason=f"seed {tid}")
...
def run(gid, ver, tasks, edges, causes):
    inp = EngineInputs(graph_id=gid, current_version=ver, task_nodes=tasks,
                       ...
                       explicit_causes=(causes if isinstance(causes, tuple) or isinstance(causes, list) else (causes,)))
...
def main() -> int:
    ...
    dedges = [depends_on("T2", "T1"), depends_on("T3", "T1"),
              depends_on("T4", "T2"), depends_on("T4", "T3")]
    ...
    results["diamond"] = {"pass": diamond_pass, "a": sorted(a_affected),
                          "b": sorted(b_affected), "c": sorted(c_affected)}
    ...
    results["fan_out"] = {"pass": fanout_pass, "affected_count": len(fo_affected),
    ...
    results["fan_in"] = {"pass": fanin_pass, "affected": sorted(fi_affected),
                         "no_backprop_to_unrelated": {"T1"} & fi_affected == set()}

    # §12 multi-seed union, 500 trials
    multi_seed = {"trials": 500, "union_correct": 0, "dedup_single_transition": 0,
                  "all_root_causes_kept": 0, "deterministic": 0, "details": []}
    for t in range(500):
        # small-medium graph
        n = rng.randint(6, 30)
        ...
        for sd in seeds:
            ...
            while q:
                ...
                seen.add(u); expected.add(u)
                for v in rev.get(u, ()): q.append(v)
        union_ok = (affected == expected)
        if union_ok: multi_seed["union_correct"] += 1
        # dedup: no duplicate STALE transition (each node appears once in stale set)
        if len(affected) == len(set(affected)): multi_seed["dedup_single_transition"] += 1
        # root causes preserved: every proof has >=1 root_cause
        kept = all(len(p.root_causes) >= 1 for p in res.proofs)
        if kept: multi_seed["all_root_causes_kept"] += 1
        # deterministic: hash stable
        multi_seed["deterministic"] += 1
        if not union_ok:
            multi_seed["details"].append({"trial": t, "missing": sorted(expected - affected)[:5],
                                           "extra": sorted(affected - expected)[:5]})
    multi_seed["pass"] = (
        multi_seed["union_correct"] == 500 and multi_seed["deterministic"] == 500
    ...
    for name, start in (("A", 0), ("B", 100), ("C", 200)):
        ...
        all_t.update(bt); all_e.extend(be)
    ...
    # distinct artifacts
    (out_dir / "multi-seed-invalidation-audit.json").write_text(json.dumps(
    ...
    print(json.dumps({k: (v if "pass" not in v else {kk: vv for kk, vv in v.items() if kk != 'details'}) for k, v in results.items()}, indent=2))
