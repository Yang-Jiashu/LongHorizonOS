...
def main() -> int:
    ...
    fresh_pass = FNode(
        "E8", bindings=(Bound("X", 8, "h8"),), source_action_id="act8", result="pass"
    )
    ...
    a_fail = evidence_applicability_for_graph(
        "g", 8, {"E8f": fail_evidence}, current_output_versions={"X": 8}
    )
    fail_applies = next(
        a for a in a_fail if a.evidence_id == "E8f"
    ).applies  # in D3 model, applicability only tracks supersession; but FAIL handling is at D1 verification, not applicability
    ...
    steps.append(
        compute_repair_frontier(
            "g",
            1,
            tasks,
            edges,
            stale_or_unverified={"T1", "T2", "T3", "T4"},
            derived_validity={"T1": "stale", "T2": "stale", "T3": "stale", "T4": "stale"},
        ).candidates
    )
    ...
    steps.append(
        compute_repair_frontier(
            ...
            derived_validity={"T1": "verified", "T2": "stale", "T3": "stale", "T4": "stale"},
        ).candidates
    )
    ...
    steps.append(
        compute_repair_frontier(
            ...
            derived_validity={"T1": "verified", "T2": "verified", "T3": "stale", "T4": "stale"},
        ).candidates
    )
    ...
    steps.append(
        compute_repair_frontier(
            ...
            derived_validity={"T1": "verified", "T2": "verified", "T3": "verified", "T4": "stale"},
        ).candidates
    )
