...
def cause(gid, ver, tid, aid): return InvalidationCause(
    cause_id=f"c:{tid}", graph_id=gid, graph_version=ver,
    cause_type="ARTIFACT_VERSION_SUPERSEDED", source_node_id=tid,
    artifact_id=aid, old_version=0, new_version=1, reason=f"seed {tid}")


def run(gid, ver, tasks, edges, causes, goals=None, goal_deps=None, evids=None,
        cur_out=None, has_active_claim=None):
    inp = EngineInputs(graph_id=gid, current_version=ver, task_nodes=tasks,
                       goal_nodes=(goals or {}), evidence_nodes=(evids or {}),
                       edges=edges, explicit_causes=causes,
                       current_output_versions=cur_out,
                       goal_direct_tasks=goal_deps, has_active_claim=has_active_claim)
...
def main() -> int:
    ...
    edges = [depends_on("T1", "T2"), depends_on("T1", "T3"),
             depends_on("T2", "T4"), depends_on("T3", "T5")]
    res = run("g", 1, tasks, edges, (cause("g", 1, "T2", "A"),))
    d1 = {"affected": list(res.stale_nodes), "preserved": list(res.preserved_nodes),
    ...
    fr2 = compute_repair_frontier("g", 2, tc, ec, stale_or_unverified={"T2", "T3"}, derived_validity=dv2)
    ...
    d2 = {"reopened_goals": list(r.reopened_goals), "initial_frontier": f1,
    ...
    d3 = {"affected": affected, "preserved": preserved,
          "affected_count": len(affected), "preserved_count": len(preserved)}
    ...
    d4 = {"E7_at_v7_applies": e7_applies,
    ...
    r5b = run("g", 1, t5, e5, (cause("g", 1, "T1", "D"),),
              has_active_claim=lambda tid: tid == "T1")
    ...
    d5 = {"initial_frontier": initial, "after_d2_claim": after_claim,
    ...
    d6 = {"crash_boundaries": list("S1S2S3S4S5"),
          "recomputed_byte_identical": byte_identical}
    assert byte_identical
    demos_out["crash-invalidation"] = d6

    # write all demo JSONs
    for name, payload in demos_out.items():
        (out_dir / f"demo-{name}.json").write_text(json.dumps({
            "demo": name, "pass": True, **payload,
        }, indent=2))
