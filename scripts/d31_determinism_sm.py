...
def _towers():
    return (
        ...
        res.cone.cone_hash,
        tuple(sorted(res.stale_nodes)),
        res.frontier.frontier_hash,
        tuple(c.task_id for c in res.frontier.candidates),
    )
...
def main() -> int:
    ...
    for e in edges_full:
        if (e.source_node_id, e.target_node_id) not in seen:
            seen.add((e.source_node_id, e.target_node_id))
    ...
    reversed_edge_ok = (res_rev.cone.cone_hash, res_rev.frontier.frontier_hash) == (
        next(iter(sigs))[0:2]
    )
    ...
    (out_dir / "determinism-results-v2.json").write_text(
        json.dumps({"spec_section": "§24", **determinism}, indent=2)
    )
            ...
            if op < 0.2:
                ...
                for cand in res.frontier.candidates:
                    for dp in cand.dependency_proof:
                        _, _, st = dp.partition(":")
                        if st != "verified":
                            violations.append(("frontier-dep-not-verified", gi, oi))
                ...
                fr = compute_repair_frontier(
                    "gs", 1, tasks_s, edges_s, stale_or_unverified=stale_set
                )
                for cand in fr.candidates:
                    for dp in cand.dependency_proof:
                        _, _, st = dp.partition(":")
                        if st != "verified":
                            violations.append(("frontier-stale-dep", gi, oi))
            ops += 1
    random_sm = {
        "graphs": 100,
        "ops_per_graph": 1000,
        ...
        "pass": ops == 100000 and len(violations) == 0,
    }
    (out_dir / "random-state-machine-v2.json").write_text(
        json.dumps({"spec_section": "§25", **random_sm}, indent=2)
    )
