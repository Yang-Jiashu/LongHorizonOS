...
- a 50 ms `asyncio.sleep` executor per task;
- three paired serial/parallel measurements, summarized by median paired speedup;
...
The report includes all serial and parallel elapsed samples, each paired
speedup, their median, peak global and per-Agent concurrency,
ownership/resource-admission violations, Claim and Attempt counts, and the
complete correctness contract.
...
The old single-sample 25 ms workload was timing-sensitive: a busy local machine
could fall below the conservative `1.5x` gate even when the correctness
contract passed. The current gate uses a 50 ms workload, alternates measurement
order, and takes the median of three paired speedups. Custom results must still
report exact parameters and host conditions because wall-clock performance is
not deterministic.
