...
WHAT). The SDK composition root now materializes and fences a bounded Context VM
snapshot for each `AgentOS.run()`/`run_async()` Attempt. Materialized
`page_bindings` are captured as exact `context_vm` provenance reads. This remains
a mediated boundary, not a sandbox or a detector for hidden reads.
