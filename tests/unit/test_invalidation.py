...
def _seed(graph_store, run_id):
    artifact = make_node(
        "art", run_id=run_id, kind=NodeKind.ARTIFACT, schedulable=False, state=NodeState.VERIFIED
    )
    ...
    graph_store.add_edge(
        GraphEdge(
            run_id=run_id, source_node_id="consumer", target_node_id="art", kind=EdgeKind.CONSUMES
        )
    )
    graph_store.add_edge(
        GraphEdge(
            run_id=run_id, source_node_id="claimed", target_node_id="art", kind=EdgeKind.CONSUMES
        )
    )
    graph_store.add_edge(
        GraphEdge(
            run_id=run_id,
            source_node_id="downstream",
...
def test_invalidation_is_recorded_as_events(graph_store, event_store, run_id):
    ...
    propagate_invalidation(graph_store, run_id, "art")
    stale_events = [
        e for e in event_store.list_events(run_id) if e.event_type == EventType.NODE_MARKED_STALE
    ]
