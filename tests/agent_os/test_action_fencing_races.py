...
class EffectThenRaiseDriver:
    """Simulate a sink applying an effect before losing its response."""

    device_type = "race/effect-then-raise"

    def __init__(self) -> None:
        self.effect_recorded = False
        self.dispatch_count = 0
    ...
    async def dispatch(
        ...
        del action_id, operation, arguments
        self.dispatch_count += 1
        self.effect_recorded = True
        raise RuntimeError("sink committed effect before acknowledgement")
...
@pytest.mark.parametrize(
    "side_effect_class",
    [SideEffectClass.NON_REVERSIBLE, SideEffectClass.UNKNOWN],
)
async def test_driver_exception_after_effect_is_uncertain_and_never_retried(
    side_effect_class: SideEffectClass,
) -> None:
    """Unknown driver failures must not be converted into a blind retry."""

    kernel = create_kernel(":memory:")
    try:
        driver = EffectThenRaiseDriver()
        kernel.register_driver(driver.device_type, driver)

        process = kernel._process_service.spawn(program_id="effect-then-raise-owner")
        action = kernel._action_service.submit(
            process.pid,
            driver.device_type,
            "external-write",
        ...
        await asyncio.wait_for(kernel._dispatch_pending_actions(), timeout=1.0)

        current = kernel._action_service.get_action(action.action_id)
        assert current is not None
        assert current.state == ActionState.UNCERTAIN
        assert current.error is not None
        assert current.error["reason"] == "driver_dispatch_exception"
        assert current.error["side_effect_class"] == side_effect_class.value
        assert driver.effect_recorded is True
        assert driver.dispatch_count == 1

        # Terminal UNCERTAIN state is not eligible for a later blind retry.
        await asyncio.wait_for(kernel._dispatch_pending_actions(), timeout=1.0)
        assert driver.dispatch_count == 1
        assert not any(
        ...
        assert any(
            event.event_type == "ACTION_UNCERTAIN"
            and event.payload.get("action_id") == action.action_id
            for event in kernel._journal.read_all()
        )
    finally:
