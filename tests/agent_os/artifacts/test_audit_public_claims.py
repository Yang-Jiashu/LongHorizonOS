...
class TestREADMEClaims:
    ...
    def test_readme_lists_implemented_features(self, claim: str) -> None:
        """README must mention every implemented Phase C1 capability.

        Matched case-insensitively: the point is that the capability is still
        described, not that it is capitalised a particular way. A case-sensitive
        match forces Title Case in the middle of an English sentence, which is a
        test dictating prose style rather than protecting a claim.
        """

        content = README.read_text(encoding="utf-8").lower()
        assert claim.lower() in content, f"README missing claim: '{claim}'"
    ...
    def test_readme_quickstart_makefile_targets(self) -> None:
        """Every ``make <target>`` the README tells a reader to run must exist.

        Only fenced code blocks and inline code are scanned. Scanning the prose
        matched the English verb -- "the facts that make the measurement mean
        anything" asserted a Makefile target named ``the`` -- and a check that
        cries wolf on ordinary sentences is one that gets ignored.
        """
        ...
        make_targets = re.findall(r"\bmake\s+(\w+)", commands)
        if make_targets:
            ...
            if makefile.exists():
                ...
                for target in set(make_targets):
                    assert f"{target}:" in makefile_content, (
                        f"Makefile missing target '{target}' referenced in README"
                    )
