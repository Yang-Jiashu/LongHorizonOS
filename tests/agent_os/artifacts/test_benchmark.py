...
def _assert_valid_timing(*, elapsed: float, operations: int) -> None:
    """Reject broken measurements without treating host speed as correctness."""

    assert operations > 0
    assert isfinite(elapsed)
    assert elapsed > 0
...
class TestBenchmarks:
    ...
    def test_timing_sequential_writes(self, setup) -> None:
        ...
        _assert_valid_timing(elapsed=elapsed, operations=n)
    ...
    def test_timing_snapshot_creation(self, setup) -> None:
        ...
        assert len(snap.artifact_versions) == n
        _assert_valid_timing(elapsed=elapsed, operations=n)
