...
class TestRestartRestore:
    ...
    def test_restore_into_new_service(self, env: dict[str, Any]) -> None:
        ...
        h2, restored = new_sdk.restore_snapshot(pid="p1", snapshot_id=snap.snapshot_id)
...
class TestSameContentAfterRestart:
    ...
    def test_content_bytes_survive_restart(self, env: dict[str, Any]) -> None:
        ...
        _h2, restored = new_sdk.restore_snapshot(pid="p1", snapshot_id=snap.snapshot_id)
...
class TestJournalReplayRebuildsWorkingSets:
    ...
    def test_journal_storage_has_context_events(self, env: dict[str, Any]) -> None:
        ...
        manifest_events = [e for e in events if e["event"] == "CONTEXT_MANIFEST_ACCEPTED"]
    ...
    def test_replay_from_journal_restores_materialized_hash(self, env: dict[str, Any]) -> None:
...
class TestTwoPhaseCrash:
    ...
    def test_snapshot_between_versions_restores_latest_written(self, env: dict[str, Any]) -> None:
        """If snapshot is taken after writing v2, restore reflects v2 (the latest written)."""
        pid = "p1"
        # Write v1
        env["artifact_sdk"].write(pid, "workspace:///evolve.md", b"version-one\n", "idem-evolve-1")
        ver = next(iter(env["artifact_sdk"].list_versions(pid, "workspace:///evolve.md")))
        arts = env["artifact_svc"].list_artifacts(pid)
        art = next(a for a in arts if a["artifact_id"] == ver["artifact_id"])

        # Load a manifest pinned to v2 (written next)
        env["artifact_sdk"].write(pid, "workspace:///evolve.md", b"version-two\n", "idem-evolve-2")
        ver2 = next(
            v
            for v in env["artifact_sdk"].list_versions(pid, "workspace:///evolve.md")
            if v["version"] == 2
        )
        ...
        _h2, restored = new_sdk.restore_snapshot(pid=pid, snapshot_id=snap.snapshot_id)
