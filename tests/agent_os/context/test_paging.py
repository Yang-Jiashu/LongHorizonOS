...
class TestStablePageId:
    ...
    def test_differs_by_position(self):
        ...
        second = _stable_page_id(
            artifact_id="a", version=1, content_hash="x" * 64,
            byte_start=10, byte_end=20, page_size=64, page_index=1)
        assert first != second
...
class TestComputePagesForRef:
    ...
    def test_contiguous_non_overlapping(self):
        ...
        pages = compute_pages_for_ref(
            ref=ref, content_supplier=_FakeSupplier(b"A" * 1000),
            estimator=self.estimator, page_size=100)
        assert len(pages) == 10
    ...
    def test_page_count_matches_ceil(self):
        for length in (1, 4, 5, 100, 101, 999, 1000, 1001):
            content = b"x" * length
            ref = ContentRef(
                ref_id="r1", canonical_uri="artifact://ns/a",
                artifact_id="aid", version=1,
                content_hash=_content_hash_for(content),
                media_type="text/plain",
            )
            pages = compute_pages_for_ref(
                ref=ref, content_supplier=_FakeSupplier(content),
                estimator=self.estimator, page_size=64)
            expected = max(1, math.ceil(length / 64))
            assert len(pages) == expected, (
                f"length={length} expected {expected} got {len(pages)}")
    ...
    def test_page_hash_matches_content_chunk(self):
        ...
        pages = compute_pages_for_ref(
            ref=ref, content_supplier=_FakeSupplier(b"A" * 1000),
            estimator=self.estimator, page_size=250)
    ...
    def test_stable_page_ids_across_invocations(self):
        ...
        a = compute_pages_for_ref(
            ref=ref, content_supplier=supplier, estimator=self.estimator,
            page_size=100)
    ...
    def test_negative_range_rejected(self):
        ref = ContentRef(
            ref_id="r", canonical_uri="artifact://ns/a",
            artifact_id="aid", version=1,
            content_hash="h" * 64, media_type="text/plain",
            start_byte=-1, end_byte=10,
        )
        with pytest.raises(ErrInvalidRange):
            compute_pages_for_ref(
                ref=ref, content_supplier=_FakeSupplier(b"A" * 100),
                estimator=self.estimator, page_size=64)
