...
class TestVersionBinding:
    ...
    def test_load_pinned_to_v2_reads_v2_content(self, env):
        c, a, versions = _write_versions(
            env=env, uri="workspace:///evolve2.md", contents=[b"v1\n", b"v2-content-data\n"]
        )
        v2 = versions[-1]
        manifest = ContextManifest(
            owner_pid=env["pid"],
    ...
    def test_version_pinning_integrity_across_writes(self, env):
        c, a, v1_versions = _write_versions(
            env=env, uri="workspace:///evolve3.md", contents=[b"original\n"]
        )
        v1 = v1_versions[-1]
        manifest = ContextManifest(
            owner_pid=env["pid"],
        ...
        env["artifact_sdk"].write(
            env["pid"], "workspace:///evolve3.md", b"later-version\n", "kv-ev3-later"
        )
    ...
    def test_different_versions_different_materialized_hash(self, env):
        pid = env["pid"]
        c, a, versions = _write_versions(
            env=env, uri="workspace:///vhash.md", contents=[b"AABB\n", b"XXYYZZ...\n"]
        )
        v1 = versions[0]
        v2 = versions[-1]
        m1 = ContextManifest(
    ...
    def test_wrong_content_hash_raises(self, env):
        c, a, versions = _write_versions(env=env, uri="workspace:///wh.md", contents=[b"good\n"])
        v = versions[-1]
        manifest = ContextManifest(
            owner_pid=env["pid"],
            refs=(
    ...
    def test_same_artifact_two_versions_in_one_manifest(self, env):
        pid = env["pid"]
        c, a, versions = _write_versions(
            env=env, uri="workspace:///mix.md", contents=[b"aaa-first\n", b"bbb-second-longer\n"]
        )
    ...
    def test_snapshot_preserves_version_binding(self, env):
        c, a, versions = _write_versions(
            env=env, uri="workspace:///snv.md", contents=[b"s1\n", b"s2-longer\n"]
        )
        v2 = versions[-1]
        manifest = ContextManifest(
            owner_pid=env["pid"],
    ...
    def test_snapshot_materialized_hash_equals_original(self, env):
        c, a, versions = _write_versions(
            env=env, uri="workspace:///mhashcheck.md", contents=[b"materialized check content\n"]
        )
        v = versions[-1]
        manifest = ContextManifest(
            owner_pid=env["pid"],
