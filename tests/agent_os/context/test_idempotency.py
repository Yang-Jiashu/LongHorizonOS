...
def _build_manifest(
    env, content, *, page_size=64, token_budget=100_000, owner_pid="p1", required=True
):
...
class TestLoadIdempotency:
    ...
    def test_same_pid_manifest_idem_returns_same_handle(self, env) -> None:
        ...
        h1, _ = env["ctx_sdk"].load(pid=env["pid"], manifest=m, idempotency_key="abc")
        h2, _ = env["ctx_sdk"].load(pid=env["pid"], manifest=m, idempotency_key="abc")
    ...
    def test_different_idem_key_produces_different_handle(self, env) -> None:
        ...
        h1, _ = env["ctx_sdk"].load(pid=env["pid"], manifest=m, idempotency_key="key-one")
        h2, _ = env["ctx_sdk"].load(pid=env["pid"], manifest=m, idempotency_key="key-two")
        assert h1.handle_id != h2.handle_id

    def test_different_pid_for_same_manifest_idem_produces_different_handle(self, env) -> None:
        ...
        h1, _ = env["ctx_sdk"].load(pid="p1", manifest=m_p1, idempotency_key="shared")
        h2, _ = env["ctx_sdk"].load(pid="p2", manifest=m_p2, idempotency_key="shared")
        assert h1.handle_id != h2.handle_id
...
class TestSnapshotIdempotency:
    ...
    def test_snapshot_same_pid_context_idem_returns_same_id(self, env) -> None:
        ...
        s1 = env["ctx_sdk"].snapshot(
            pid=env["pid"], context_id=loaded.context_id, idempotency_key="snap-x"
        )
    ...
    def test_restore_same_pid_idem_returns_same_handle_id(self, env) -> None:
        ...
        snap = env["ctx_sdk"].snapshot(
            pid=env["pid"], context_id=loaded.context_id, idempotency_key="snap-y"
        )
        r1h, _ = env["ctx_sdk"].restore_snapshot(
            pid=env["pid"], snapshot_id=snap.snapshot_id, idempotency_key="rid-1"
        )
    ...
    def test_snapshot_after_content_change_produces_different_id(self, env) -> None:
        ...
        snap1 = env["ctx_sdk"].snapshot(
            pid=env["pid"], context_id=l1.context_id, idempotency_key="idem-z"
        )
        h2, l2 = _mk(env, b"idempotent snapshot content v2-beta")
        snap2 = env["ctx_sdk"].snapshot(
            pid=env["pid"], context_id=l2.context_id, idempotency_key="idem-z"
        )
...
class TestCloseIdempotency:
    ...
    def test_second_close_succeeds(self, env) -> None:
        ...
        first = env["ctx_sdk"].close(pid=env["pid"], handle_id=handle.handle_id)
        assert first is True
        second = env["ctx_sdk"].close(pid=env["pid"], handle_id=handle.handle_id)
...
class TestPinUnpinIdempotency:
    ...
    def test_pin_same_page_twice_still_pinned_once(self, env) -> None:
        ...
        first_pin = env["ctx_sdk"].pin(
            pid=env["pid"], handle_id=handle.handle_id, page_ids=[pages[0]]
        )
...
class TestEvictionIdempotency:
    ...
    def test_evict_already_evicted_ws_is_noop_or_empty(self, env) -> None:
        ...
        empty evicted_pages."""
        handle, _loaded = _mk(env, b"evict idempotency content " * 2, page_size=64)
