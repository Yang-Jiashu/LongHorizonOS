...
class SlowPublisher:
    """Publisher whose completion occurs after the outbox claim TTL."""
    ...
    async def publish(
        ...
        *,
        ...
        del destination, payload, idempotency_key, headers
        self.calls += 1
        import asyncio

        await asyncio.sleep(self.delay_seconds)
        return {"receipt": f"slow-{self.calls}"}


class YieldingFailurePublisher:
    """Failure publisher that yields once before raising.

    The yield makes the dispatcher observe non-zero wall-clock elapsed time
    even when callers provide a fixed logical ``now``.  A zero retry delay
    must still make the row immediately eligible at that logical timestamp.
    """
...
@pytest.mark.asyncio
async def test_slow_publisher_cannot_ack_after_claim_ttl(
    storage: SQLiteStorage,
) -> None:
    """A completion after lease expiry must be reported as unacknowledged."""

    outbox = TransactionalOutbox(storage)
    record = outbox.enqueue(_intent(), now=NOW)
    publisher = SlowPublisher(delay_seconds=0.03)

    result = await outbox.dispatch_once(
        publisher,
        worker_id="slow-worker",
        now=NOW,
        claim_ttl=timedelta(milliseconds=5),
    )

    assert result.delivered is False
    assert result.error == "outbox claim lost before acknowledgement"
    current = outbox.get(record.outbox_id)
    assert current is not None
    assert current.status == OutboxStatus.IN_FLIGHT
...
async def test_retryable_failure_survives_and_is_redelivered(
    ...
    # ``dispatch_once`` adds the publisher's *real* monotonic elapsed time to the
    # injected logical clock, so the retry becomes available at
    # ``NOW + real_elapsed + retry_delay`` rather than exactly ``NOW + 5s``.
    # Under load that real elapsed time is not negligible, so a hard-coded
    # ``NOW + 5s`` intermittently lands *before* the retry is due.  Read the
    # record's own availability instead of assuming it.
    retry_due = pending.available_at
    assert retry_due is not None
    assert retry_due >= NOW + timedelta(seconds=5)
...
@pytest.mark.asyncio
async def test_zero_retry_delay_is_immediately_eligible_after_async_failure(
    storage: SQLiteStorage,
) -> None:
    """A fixed logical clock must not turn zero-delay retry into a missed retry."""

    outbox = TransactionalOutbox(storage)
    record = outbox.enqueue(_intent(), now=NOW)
    publisher = YieldingFailurePublisher()
    ...
    assert pending is not None
    assert pending.status == OutboxStatus.PENDING
    assert pending.available_at == NOW
