...
def test_owner_pid_is_cached_before_agents_are_registered():
    os_ = AgentOS(":memory:")
    try:
        before = len(os_.kernel._process_service.list_all())
        first = os_._owner_pid()
        middle = len(os_.kernel._process_service.list_all())
        second = os_._owner_pid()
        after = len(os_.kernel._process_service.list_all())
