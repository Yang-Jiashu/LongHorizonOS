...
def _agent(aid, cost, specs=("python",)):
    return AgentDescriptor(
        agent_id=aid, process_id=f"pid-{aid}",
        supported_task_kinds=("*",),
        specializations=specs, cost_weight=cost,
    )
...
def test_same_pool_same_hash_regardless_of_order():
    ...
    d1 = match_deterministic_best_fit_v1(
        ...
        eligible_agents=pool_a, active_claims_by_agent={},
        preferred_specializations=("python",),
    )
    d2 = match_deterministic_best_fit_v1(
        graph_id="g", graph_version=1, task_id="t", task_priority=0,
        eligible_agents=pool_b, active_claims_by_agent={},
        preferred_specializations=("python",),
    )
...
def test_candidates_vector_order_deterministic():
    ...
    d = match_deterministic_best_fit_v1(
        graph_id="g", graph_version=1, task_id="t", task_priority=0,
        eligible_agents=pool, active_claims_by_agent={},
    )
...
def test_repeat_runs_identical_hash():
    ...
    for _ in range(10):
        d = match_deterministic_best_fit_v1(
            graph_id="g", graph_version=2, task_id="t", task_priority=1,
...
def test_decision_hash_changes_with_inputs():
    ...
    d_base = match_deterministic_best_fit_v1(
        graph_id="g", graph_version=1, task_id="t", task_priority=0,
        eligible_agents=base_pool, active_claims_by_agent={},
    )
    d_priority = match_deterministic_best_fit_v1(
        graph_id="g", graph_version=1, task_id="t", task_priority=5,
