...
def test_rejects_disabled_agent_when_process_live():
    ...
    r = evaluate_eligibility(
        agent, "t1", "g1", 1,
        task_kind="", required_specializations=(), required_tools=(),
        required_capabilities=(), readiness_version=1,
        active_claims_for_agent=0, process_state="running",
...
def test_rejects_exited_process_due_to_liveness():
    ...
    r = evaluate_eligibility(
        ...
        task_kind="", required_specializations=(), required_tools=(),
        required_capabilities=(), readiness_version=1,
        active_claims_for_agent=0, process_state="exited",
        process_exists=True, capability_checker=None,
    )
    assert not r.eligible
...
def test_rejects_capacity_exhaustion():
    ...
    r = evaluate_eligibility(
        ...
        task_kind="", required_specializations=(), required_tools=(),
        required_capabilities=(), readiness_version=1,
        active_claims_for_agent=1, process_state="running",
        process_exists=True, capability_checker=None,
    )
    assert not r.eligible
...
def test_rejects_missing_specialization():
    ...
    r = evaluate_eligibility(
        agent, "t1", "g1", 1,
        task_kind="", required_specializations=("python",),
        required_tools=(), required_capabilities=(),
        readiness_version=1, active_claims_for_agent=0,
        process_state="running", process_exists=True,
        capability_checker=None,
    )
    assert not r.eligible
...
def test_rejects_missing_tool():
    ...
    r = evaluate_eligibility(
        agent, "t1", "g1", 1,
        task_kind="", required_specializations=(),
        required_tools=("shell",), required_capabilities=(),
        readiness_version=1, active_claims_for_agent=0,
        process_state="running", process_exists=True,
        capability_checker=None,
    )
...
def test_kind_wildcard_matches_anything():
    ...
    r = evaluate_eligibility(
        agent, "t1", "g1", 1,
        task_kind="some_random_kind", required_specializations=(),
        required_tools=(), required_capabilities=(),
        readiness_version=1, active_claims_for_agent=0,
        process_state="running", process_exists=True,
        capability_checker=None,
...
def test_required_task_kind_must_be_supported():
    ...
    r = evaluate_eligibility(
        agent, "t1", "g1", 1,
        task_kind="code_review", required_specializations=(),
        required_tools=(), required_capabilities=(),
        readiness_version=1, active_claims_for_agent=0,
        process_state="running", process_exists=True,
        capability_checker=None,
...
def test_determinism_across_reason_order():
    ...
    agent = _agent("a1", "p1")
    args = dict(
        task_id="t1", graph_id="g1", graph_version=1,
        task_kind="coding",
        required_specializations=("python", "rust"),
        required_tools=("shell",),
        required_capabilities=("device:tool/mock:invoke",),
        readiness_version=1, active_claims_for_agent=5,
        process_state="exited", process_exists=True,
        capability_checker=None,
    )
