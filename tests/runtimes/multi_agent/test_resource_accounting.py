...
def test_snapshot_is_deterministic_and_lock_consistent() -> None:
    manager = AtomicResourceManager(
        {
            "agent-b": ResourceVector(cpu_millis=2_000),
            "agent-a": ResourceVector(
                cpu_millis=4_000,
                ram_bytes=8_000,
    ...
    first = manager.try_reserve(
        ...
        request=ResourceVector(cpu_millis=500, ram_bytes=1_000, model_slots={"reasoner": 1}),
    )
    second = manager.try_reserve(
        pool_id="agent-a",
        owner_id="claim-a",
        reservation_id="reservation-a",
        request=ResourceVector(cpu_millis=1_000, ram_bytes=500),
    )
    assert first is not None and second is not None

    snapshot = manager.snapshot()
    assert [item.pool_id for item in snapshot] == ["agent-a", "agent-b"]
    agent_a = snapshot[0]
    assert agent_a.capacity == ResourceVector(
        cpu_millis=4_000,
        ram_bytes=8_000,
        model_slots={"reasoner": 2},
    )
    assert agent_a.used == ResourceVector(
        cpu_millis=1_500,
        ram_bytes=1_500,
        model_slots={"reasoner": 1},
    )
    assert agent_a.available == ResourceVector(
        cpu_millis=2_500,
        ram_bytes=6_500,
        model_slots={"reasoner": 1},
    )
    assert [item.reservation_id for item in agent_a.reservations] == [
        "reservation-a",
        "reservation-b",
    ]
    assert snapshot == manager.snapshot()
    assert manager.snapshot("agent-a") == (agent_a,)
    assert manager.snapshot("missing") == ()


def test_snapshot_rejects_internal_overcommit_instead_of_publishing_negative_available() -> None:
    manager = AtomicResourceManager({"agent": ResourceVector(cpu_millis=1_000)})
    reservation = manager.try_reserve(
        pool_id="agent",
        owner_id="claim",
    ...
    assert reservation is not None
    # Simulate a malformed in-memory projection; public mutation paths cannot
    # create this state, but a fail-closed projection must not expose a
    # negative available vector if durable state is corrupted.
    with manager._lock:
        manager._reservations[reservation.reservation_id] = reservation.model_copy(
            update={"resources": ResourceVector(cpu_millis=2_000)}
        )
