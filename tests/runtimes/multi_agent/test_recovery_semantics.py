...
class _MalformedTokenCleanupLeases(_TrackingLeases):
    """Acquisition returns malformed fencing data; first cleanup is uncertain."""
...
def test_malformed_acquisition_release_failure_remains_reconcilable() -> None:
    vpg = _ready()
    leases = _MalformedTokenCleanupLeases()
    scheduler = _scheduler(vpg, leases)

    # The provider granted a lease, but its fencing token is malformed and the
    # first compensating release is not confirmed.  The scheduler surfaces the
    # acquisition failure while retaining a non-terminal claim for recovery.
    with pytest.raises(Exception, match="malformed fencing token"):
        scheduler.schedule_once(vpg.graph_id)

    claim = scheduler.claims[-1]
    assert claim.state == ClaimState.ACQUIRING
    assert claim.lease_id is not None
    assert leases._leases

    reconciled = scheduler.reconcile()
    assert reconciled.claims_marked_lost == 1
    assert claim.state == ClaimState.LOST
    assert leases._leases == {}
    assert leases.release_attempts == 2
    assert scheduler._s.resource_manager.for_owner(claim.claim_id) is None
