...
# ── helpers ──────────────────────────────────────────────────────────────
def _agent(**kw):
    defaults = dict(agent_id="a", process_id="p", supported_task_kinds=("*",),
                    specializations=(), max_concurrency=1, cost_weight=100,
                    enabled=True)
    defaults.update(kw)
...
def test_m01_disabled_agent_always_ineligible():
    ...
    r = evaluate_eligibility(a, "t", "g", 1, task_kind="x",
                             required_specializations=(), required_tools=(),
                             required_capabilities=(), readiness_version=1,
                             active_claims_for_agent=0, process_state="ready",
                             process_exists=True, capability_checker=None)
...
def test_m02_nonexistent_process_rejected():
    ...
    r = evaluate_eligibility(a, "t", "g", 1, task_kind="x",
                             required_specializations=(), required_tools=(),
                             required_capabilities=(), readiness_version=1,
                             active_claims_for_agent=0, process_state=None,
                             process_exists=False, capability_checker=None)
    assert not r.eligible
...
def test_m03_terminal_process_rejected():
    ...
    for term in ("exited", "failed"):
        r = evaluate_eligibility(a, "t", "g", 1, task_kind="x",
                                 required_specializations=(), required_tools=(),
                                 required_capabilities=(), readiness_version=1,
                                 active_claims_for_agent=0, process_state=term,
                                 process_exists=True, capability_checker=None)
        assert not r.eligible
...
def test_m04_capacity_exhausted_rejected():
    ...
    r = evaluate_eligibility(a, "t", "g", 1, task_kind="x",
                             required_specializations=(), required_tools=(),
                             required_capabilities=(), readiness_version=1,
                             active_claims_for_agent=1, process_state="ready",
                             process_exists=True, capability_checker=None)
    assert not r.eligible
...
def test_m05_missing_specialization_rejected():
    ...
    r = evaluate_eligibility(a, "t", "g", 1, task_kind="x",
                             ...
                             readiness_version=1, active_claims_for_agent=0,
                             process_state="ready", process_exists=True,
...
def test_m07_missing_kernel_capability_rejected():
    ...
    assert not r.eligible
    assert any("kernel capability" in x.lower() or "missing kernel" in x.lower()
               for x in r.reasons)


def test_m08_task_kind_mismatch_rejected():
    a = _agent(supported_task_kinds=("code_review", "test"))
    r = evaluate_eligibility(a, "t", "g", 1, task_kind="deploy",
                             required_specializations=(), required_tools=(),
                             required_capabilities=(), readiness_version=1,
                             active_claims_for_agent=0, process_state="ready",
                             process_exists=True, capability_checker=None)
...
def test_m10_matching_deterministic():
    pool = [_agent(agent_id="a", cost_weight=100),
            _agent(agent_id="b", cost_weight=50)]
    d = match_deterministic_best_fit_v1(
        graph_id="g", graph_version=1, task_id="t", task_priority=0,
        eligible_agents=copy.deepcopy(pool), active_claims_by_agent={},
    )
...
def test_m11_matching_insertion_order_invariance():
    ...
    d1 = match_deterministic_best_fit_v1(
        graph_id="g", graph_version=1, task_id="t", task_priority=0,
        eligible_agents=copy.deepcopy(one), active_claims_by_agent={})
    d2 = match_deterministic_best_fit_v1(
        graph_id="g", graph_version=1, task_id="t", task_priority=0,
        eligible_agents=copy.deepcopy(two), active_claims_by_agent={})
    assert d1.decision_hash == d2.decision_hash


def test_m12_i4_detect_multi_active_per_task():
    claims = [
        TaskClaim(graph_id="g", graph_version=1, task_id="t", agent_id="a1",
                  process_id="p1", lease_resource="r", state=ClaimState.ACTIVE),
        TaskClaim(graph_id="g", graph_version=1, task_id="t", agent_id="a2",
                  process_id="p2", lease_resource="r", state=ClaimState.ACTIVE),
    ]
    v = detect_invariants_violations(claims,
...
def test_m13_i5_active_claim_requires_lease_id():
    claims = [
        TaskClaim(graph_id="g", graph_version=1, task_id="t", agent_id="a",
                  process_id="p", lease_resource="r", state=ClaimState.ACTIVE,
                  lease_id=None),
    ]
    v = detect_invariants_violations(claims,
                                     lease_is_live=lambda lid: True,
...
def test_m16_active_claim_count_helper():
    claims = [
        TaskClaim(graph_id="g", graph_version=1, task_id="t1", agent_id="a",
                  process_id="p", lease_resource="r", state=ClaimState.ACTIVE),
        TaskClaim(graph_id="g", graph_version=1, task_id="t2", agent_id="a",
                  process_id="p", lease_resource="r", state=ClaimState.ACTIVE),
        TaskClaim(graph_id="g", graph_version=1, task_id="t3", agent_id="a",
                  process_id="p", lease_resource="r", state=ClaimState.COMPLETED),
        TaskClaim(graph_id="g", graph_version=1, task_id="t4", agent_id="b",
                  process_id="q", lease_resource="r", state=ClaimState.ACTIVE),
    ]
...
def test_m17_claim_state_enum_complete():
    ...
    for s in ("proposed", "acquiring", "active", "released", "lost",
              "completed", "rejected"):
        assert s in values
...
def test_m20_drei_plus_states_are_terminal():
    ...
    for s in (ClaimState.COMPLETED, ClaimState.LOST, ClaimState.RELEASED,
              ClaimState.REJECTED):
