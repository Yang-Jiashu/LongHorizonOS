...
def test_multi_seed_proof_has_root_cause(run_engine, cause):
    ...
    tasks = {
        "A0": TNode("A0"), "A1": TNode("A1"),   # A1 depends on A0
        "B0": TNode("B0"), "B1": TNode("B1"),   # B1 depends on B0
    }
...
def test_deep_chain_proof_path_reaches_seed(run_engine, cause):
    ...
    tasks = {
        "T1": TNode("T1"), "T2": TNode("T2"), "T3": TNode("T3"),
    }
    edges = [depends_on("T2", "T1"), depends_on("T3", "T2")]
    res = run_engine(tasks=tasks, edges=edges,
...
def test_proof_path_terminates_at_affected_node(run_engine, cause):
    ...
    res = run_engine(tasks=tasks, edges=edges,
                     explicit_causes=(cause(source_node_id="A0"),))
    for p in res.proofs:
