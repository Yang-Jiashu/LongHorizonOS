...
class TestVPG01:
    ...
    def _build_verified_rt(self):
        ...
        gid2 = rec.graph_id
        _patch(rt2, gid2, "s", (
            AddNodeOp(node_id="t1", graph_id=gid2, node_type="task",
                       created_by_pid="p1", title="T"),
            AddNodeOp(node_id="v1", graph_id=gid2, node_type="verification",
                       created_by_pid="p1"),
        ...
        evi = EvidenceNode(
            graph_id=gid2, node_id="evi1", node_type=NodeType.EVIDENCE,
...
class TestVPG04:
    def test_conflicting_patch_atomic_rollback(self, g):
        ...
        stale = GraphPatchProposal(
            ...
            operations=(
                AddNodeOp(node_id="TX", graph_id=gid, node_type="task",
    ...
    def test_self_loop_rejected(self, g):
        ...
        with pytest.raises(VPGError):
            _patch(rt, gid, "self", (
                AddNodeOp(node_id="t1", graph_id=gid, node_type="task",
                           created_by_pid="p1", title="T"),
                AddEdgeOp(edge_id="self", edge_type="depends_on",
...
class TestVPG06:
    def _build_setup_rt(self, action_id="ghost", result="pass"):
        ...
        evi = EvidenceNode(
            ...
            source_verification_id="v1", source_action_id=action_id,
            produced_by_pid="p1",
    ...
    def test_validator_directly_rejects_missing_action(self, g):
        ...
        evi_list = [n for n in rt.store.get_all_nodes(gid)
        ...
        v1 = next(n for n in rt.store.get_all_nodes(gid)
                  if n.node_type == NodeType.VERIFICATION)
        prod_edge = __import__(
            ...
            source_node_id="evi1", target_node_id=v1.node_id,
...
class TestVPG07:
    def test_validator_directly_rejects_hash_mismatch(self, g):
        ...
        rt = VerifiedProgressRuntime(":memory:",
                                     facts_artifact=facts_check,
                                     facts_kernel=facts_check)
        rec = rt.create_graph(owner_pid="p1")
        gid = rec.graph_id
        _patch(rt, gid, "s", (
            AddNodeOp(node_id="t1", graph_id=gid, node_type="task",
        ...
        evi = EvidenceNode(
            ...
            created_by_pid="p1",
            artifact_bindings=(
                ArtifactVersionBinding(canonical_uri="u", artifact_id="a",
                                       version=1, content_hash="wrong"),
            ),
        )
        rt.store.conn.execute(
            "INSERT INTO graph_nodes_projection "
    ...
    def test_no_verified_on_hash_mismatch(self, g):
        ...
        rt.store.conn.commit()
        b = ArtifactVersionBinding(canonical_uri="u", artifact_id="a",
                                   version=1, content_hash="correct")
        _patch(rt, gid, "art", (
            AttachArtifactOp(task_node_id="t1", artifact=b,
                             created_by_pid="p1", edge_id="p1"),
        ))
...
class TestVPG08:
    def test_validator_directly_rejects_fail(self, g):
        ...
        evi = EvidenceNode(
            graph_id=gid, node_id="evi1", node_type=NodeType.EVIDENCE,
            evidence_kind="command_result", result="fail",
            source_verification_id="v1", source_action_id="act1",
            produced_by_pid="p1",
            created_in_version=rt.get_graph(gid).current_version,
            updated_in_version=rt.get_graph(gid).current_version,
...
class TestVPG10:
    def test_goal_does_not_close_when_only_one_of_two_tasks_verified(self):
        ...
        def add_task(task_id, title="T"):
            _patch(rt, gid, f"add-{task_id}", (
                AddNodeOp(node_id=task_id, graph_id=gid, node_type="task",
                           created_by_pid="p1", title=title),
                AddNodeOp(node_id=f"v-{task_id}", graph_id=gid,
                           node_type="verification", created_by_pid="p1"),
                AddEdgeOp(edge_id=f"vf-{task_id}", edge_type="verifies",
                           source_node_id=f"v-{task_id}",
                           target_node_id=task_id,
                           created_by_pid="p1"),
            ))
            _patch(rt, gid, f"dep-{task_id}", (
                AddEdgeOp(edge_id=f"dep-{task_id}", edge_type="depends_on",
                           source_node_id="g1", target_node_id=task_id,
                           created_by_pid="p1"),
            ))
        ...
        goal_closed = [
            e for e in events
            if e.event_type == GraphEventType.GOAL_CLOSED_DERIVED
        ]
        assert not goal_closed, (
            "Goal closed with only one of two tasks verified"
        )
    ...
    def test_goal_closes_when_all_tasks_verified(self):
        ...
        assert goal_closed, (
            "Goal must close when all dep tasks verified"
        )
...
class TestVPG11:
    def _setup_v1_verified_rt(self):
        ...
        _patch(rt, gid, "s", (
            ...
            AddNodeOp(node_id="v1", graph_id=gid, node_type="verification",
                       created_by_pid="p1"),
            AddEdgeOp(edge_id="vf", edge_type="verifies",
                       source_node_id="v1", target_node_id="t1",
                       created_by_pid="p1"),
        ))
    ...
    def test_v2_attach_rejected_when_binding_unknown(self, g):
        ...
        strict.verify_binding = (
            lambda p, b: b.version == 1 and b.content_hash == "h1"
        )
...
class TestVPG13:
    def test_admit_rejects_node_with_wrong_graph_id(self, g):
        ...
        bad = TaskNode(
            ...
            node_id="bad", node_type=NodeType.TASK,
            created_by_pid="p1",
            created_in_version=1, updated_in_version=1,
            title="X",
        )
        res = admit(bad, gid)
        ...
        for p in VP.rglob("*.py"):
            ...
            for line in text.splitlines():
                if pat.search(line) and not line.strip().startswith("#"):
                    offenders.append(f"{p.name}: {line.strip()}")
        assert not offenders, (
            "VPG must not reference kernel types:\n" + "\n".join(offenders)
        )
