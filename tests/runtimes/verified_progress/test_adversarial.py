    ...
    def test_same_node_id_twice_rejected(self, g):
        ...
        with pytest.raises(VPGError):
            _patch(
                ...
                (
                    AddNodeOp(
                        node_id="t1", graph_id=gid, node_type="task", created_by_pid="p1", title="A"
                    ),
                    AddNodeOp(
                        node_id="t1", graph_id=gid, node_type="task", created_by_pid="p1", title="B"
                    ),
                ),
            )
    ...
    def test_unknown_node_type_rejected(self, g):
        ...
        with pytest.raises(VPGError):
            _patch(
                ...
                (
                    AddNodeOp(
                        node_id="x", graph_id=gid, node_type="not_a_type", created_by_pid="p1"
                    ),
                ),
            )
    ...
    def test_stale_version_rejected_five_times(self, g):
        ...
        for i in range(5):
            stale = GraphPatchProposal(
                ...
                operations=(
                    AddNodeOp(node_id=f"x{i}", graph_id=gid, node_type="task", created_by_pid="p1"),
                ),
            )
            with pytest.raises(VPGError):
...
class TestAdv15_AddNodeTwiceDifferentPatches:
    def test_re_add_same_node_id_rejected_or_noop(self, g):
        ...
        all_t1 = [n for n in rt.store.get_all_nodes(gid) if n.node_id == "t1"]
        assert len(all_t1) == 1
...
class TestAdv16_IdempotencyKeyDifferentOps:
    def test_same_idempotency_key_later_diff_ops_deduplicated(self, g):
        ...
        _patch(
            ...
            (
                AddNodeOp(
                    node_id="zzz", graph_id=gid, node_type="task", created_by_pid="p1", title="Z"
                ),
            ),
        )
...
class TestAdv17_OptimisticLockAfterCommits:
    def test_stale_version_after_five_commits(self, g):
        ...
        stale = GraphPatchProposal(
            ...
            operations=(
                AddNodeOp(node_id="conflict", graph_id=gid, node_type="task", created_by_pid="p1"),
            ),
        )
        with pytest.raises(VPGError):
            rt.submit_patch(stale)
    ...
    def test_shared_task_does_not_close_both_goals_prematurely(self, g):
        ...
        _patch(
            ...
            (
                ...
                AddNodeOp(
                    node_id="g2", graph_id=gid, node_type="goal", created_by_pid="p1", title="G2"
                ),
                AddNodeOp(
                    node_id="shared",
                    graph_id=gid,
        ...
        def verify(task_id):
            ...
            _patch(
                ...
                (
                    AddNodeOp(
                        node_id=f"v_{task_id}",
                        graph_id=gid,
                        node_type="verification",
                        created_by_pid="p1",
                    ),
                    AddEdgeOp(
                        edge_id=f"vf_{task_id}",
                        edge_type="verifies",
            ...
            _patch(
                ...
                (
                    AttachEvidenceOp(
                        verification_node_id=f"v_{task_id}",
...
class TestAdv24_EmptyContentHash:
    def test_evidence_with_empty_hash_from_strict_provider(self, g):
        ...
        b = ArtifactVersionBinding(canonical_uri="u", artifact_id="a", version=1, content_hash="")
        evi = EvidenceNode(
            graph_id=gid,
            node_id="evi1",
            node_type=NodeType.EVIDENCE,
...
class TestAdv28_StaleRepinAfterVerify:
    def test_stale_artifact_repin(self):
        ...
        _patch(
            ...
            gid,
            "art2",
            (AttachArtifactOp(task_node_id="t1", artifact=b2, created_by_pid="p1", edge_id="p2"),),
        )
...
class TestAdv29_MultipleArtifactsBound:
    def test_two_artifact_pins_to_same_task(self, g):
        ...
        b1 = ArtifactVersionBinding(
            canonical_uri="u1", artifact_id="a", version=1, content_hash="h1"
        )
        b2 = ArtifactVersionBinding(
            canonical_uri="u2", artifact_id="b", version=1, content_hash="h2"
        )
        _patch(
            rt,
            gid,
