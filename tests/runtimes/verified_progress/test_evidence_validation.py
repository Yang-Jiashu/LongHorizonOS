...
class TestEvidenceResultCodes:
    def test_provenance_digest_must_match_report_hash(self):
        nodes, edges = _verification_nodes_edges()
        digest = "a" * 64
        ev = _evidence(result="pass", source_action_id="act1").model_copy(
            update={
                "provenance_digest": digest,
                "metadata": {"provenance": {"report_hash": "b" * 64}},
            }
        )
        res = validate_evidence(
            ...
            facts_artifact=_FakeArtifact(),
            facts_kernel=_FakeKernel(_FakeAction()),
        )
        assert res.valid is False
        assert res.code == VPGCode.EVIDENCE_PROVENANCE_DIGEST_MISMATCH

    def test_matching_provenance_digest_does_not_block_valid_chain(self):
        digest = "a" * 64
        v1 = VerificationNode(
            graph_id="G",
            node_id="v1",
        ...
        ev = _evidence(
            ...
            update={
                "provenance_digest": digest,
                "metadata": {"provenance": {"report_hash": digest}},
            }
        )
    ...
    def test_pass_action_not_committed_invalid(self):
        ...
        res = validate_evidence(
            ...
            existing_edges=edges,
            facts_artifact=_FakeArtifact(),
            facts_kernel=_FakeKernel(_FakeAction(state="running")),
        )
        assert res.valid is False
    ...
    def test_foreign_pid_action_rejected(self):
        """An action from a process pid != evidence.produced_by_pid must NOT
        verify the evidence — the action must belong to the process that
        ...
        ev = EvidenceNode(
            ...
            result="pass",
            source_verification_id="v1",
            source_action_id="act-other-proc",
            produced_by_pid="p1",
            created_in_version=0,
            updated_in_version=0,
            created_by_pid="p1",
        ...
        res = validate_evidence(
            ...
            existing_nodes=nodes,
            existing_edges=edges,
            facts_artifact=_FakeArtifact(verify=True),
            facts_kernel=_FakeKernel(_FakeAction(pid="other-process")),
        )
        assert res.valid is False
