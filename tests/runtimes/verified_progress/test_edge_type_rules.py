...
class TestInvalidCombinations:
        ...
        with pytest.raises(VPGError) as ei:
            _p(rt, gid, "ga-ar", (
                ...
                AddEdgeOp(edge_id="e1", edge_type="depends_on",
                          source_node_id="g1", target_node_id="ar1", created_by_pid="p1"),
            ))
    ...
    def test_goal_to_goal_rejected(self, graph):
        ...
        with pytest.raises(VPGError) as ei:
            _p(rt, gid, "gg", (
                AddNodeOp(node_id="g1", graph_id=gid, node_type="goal", created_by_pid="p1", title="G"),
                AddNodeOp(node_id="g2", graph_id=gid, node_type="goal", created_by_pid="p1", title="G2"),
                AddEdgeOp(edge_id="e1", edge_type="depends_on",
                          source_node_id="g1", target_node_id="g2", created_by_pid="p1"),
            ))
        assert ei.value.code == VPGCode.INVALID_EDGE_TYPE_COMBINATION
    ...
    def test_goal_depends_on_itself_rejected(self, graph):
        ...
        with pytest.raises(VPGError) as ei:
            _p(rt, gid, "gself", (
                AddNodeOp(node_id="g1", graph_id=gid, node_type="goal", created_by_pid="p1", title="G"),
                AddEdgeOp(edge_id="e1", edge_type="depends_on",
                          source_node_id="g1", target_node_id="g1", created_by_pid="p1"),
            ))
        assert ei.value.code == VPGCode.GRAPH_EXECUTION_CYCLE
