...
class _Facts:
    ...
    def list_events_for_pid(self, pid): return []
    def artifact_exists(self, pid, uri, ver): return True
    def read_hash(self, pid, uri, ver): return None
    def verify_binding(self, pid, b): return True
    def can_read(self, pid, aid, ver): return True
...
class TestGoalClosureAllDeps:
    ...
    def _g(self, graph):
        ...
        _patch(self.rt, self.gid, "setup", (
            AddNodeOp(node_id="g1", graph_id=self.gid, node_type="goal", created_by_pid="p1", title="G"),
            AddNodeOp(node_id="t1", graph_id=self.gid, node_type="task", created_by_pid="p1", title="T1"),
            AddNodeOp(node_id="t2", graph_id=self.gid, node_type="task", created_by_pid="p1", title="T2"),
            AddEdgeOp(edge_id="d1", edge_type="depends_on", source_node_id="g1", target_node_id="t1", created_by_pid="p1"),
            AddEdgeOp(edge_id="d2", edge_type="depends_on", source_node_id="g1", target_node_id="t2", created_by_pid="p1"),
        ))

    def _verify_task(self, tid, aid, uri="u", version=1, hash_="h"):
        bind = ArtifactVersionBinding(canonical_uri=uri, artifact_id="a", version=version, content_hash=hash_)
        vnode_id = f"v_{tid}"
        evi_id = f"evi_{tid}"
        _patch(self.rt, self.gid, f"vfy_{tid}_nodes", (
            AddNodeOp(node_id=vnode_id, graph_id=self.gid, node_type="verification", created_by_pid="p1", verification_kind="command_result"),
            AddEdgeOp(edge_id=f"vf_{tid}", edge_type="verifies", source_node_id=vnode_id, target_node_id=tid, created_by_pid="p1"),
        ))
        evi = EvidenceNode(graph_id=self.gid, node_id=evi_id, node_type=NodeType.EVIDENCE,
            ...
            updated_in_version=self.rt.get_graph(self.gid).current_version, created_by_pid="p1",
        ...
        self.rt.store.conn.execute(
            ...
            (evi_id, self.gid, "evidence", evi.model_dump_json())); self.rt.store.conn.commit()
        _patch(self.rt, self.gid, f"art_{tid}",
            (AttachArtifactOp(task_node_id=tid, artifact=bind, created_by_pid="p1", edge_id=f"p_{tid}"),))
        _patch(self.rt, self.gid, f"att_{tid}",
            (AttachEvidenceOp(verification_node_id=vnode_id, evidence_node_id=evi_id, created_by_pid="p1", edge_id=f"pe_{tid}"),))
    ...
    def test_empty_goal_does_not_auto_close(self, graph):
        gid, rt = graph
        _patch(rt, gid, "setup", (
            AddNodeOp(node_id="g1", graph_id=gid, node_type="goal", created_by_pid="p1", title="NoDeps"),
        ))
        evts = _etypes(rt, gid)
        assert GraphEventType.GOAL_CLOSED_DERIVED not in evts
    ...
    def test_goal_reopens_when_dep_goes_stale(self):
        ...
        bind2 = ArtifactVersionBinding(canonical_uri="u", artifact_id="a", version=2, content_hash="h2")
        _patch(self.rt, self.gid, "t1_new_version",
            (AttachArtifactOp(task_node_id="t1", artifact=bind2, created_by_pid="p1", edge_id="p1b"),))
        new_evts = [e.event_type for e in self.rt.get_events(self.gid) if e.event_type in (GOAL_REOPENED_DERIVED,)]
