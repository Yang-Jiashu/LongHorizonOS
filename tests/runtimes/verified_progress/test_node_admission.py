...
class TestGoalRequiresTitle:
    def test_goal_without_title_rejected(self, graph):
        ...
        with pytest.raises(VPGError):
            _p(
                ...
                (
                    AddNodeOp(
                        node_id="g1", graph_id=gid, node_type="goal", created_by_pid="p1", title=""
                    ),
                ),
            )
    ...
    def test_goal_with_title_admitted(self, graph):
        ...
        _p(
            ...
            (
                AddNodeOp(
                    node_id="g1", graph_id=gid, node_type="goal", created_by_pid="p1", title="Goal"
                ),
            ),
        )
            ...
            GraphPatchProposal(
                ...
                operations=(
                    AddNodeOp(
                        ...
                        node_type="task",
                        created_by_pid="p1",
                        execution_spec={"bin": "/usr/local/bin/evil"},
                    ),
                ),
            )
