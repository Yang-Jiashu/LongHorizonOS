...
class TestMaterializedCacheIntegrity:
    def test_commit_parent_cache_is_bounded_to_latest_version(self, base_store):
        """The hot-path cache must not recreate O(V²) retained projections."""

        store, gid, rt = base_store
        assert list(store._projection_cache) == [(gid, rt.get_graph(gid).current_version)]
        ...
        for i in range(4):
            ...
            rt.submit_patch(
                GraphPatchProposal(
                    ...
                    author_pid="p1",
                    idempotency_key=f"cache-bound-{i}",
                    operations=(
                        AddNodeOp(
                            node_id=f"cache-bound-{i}",
                            graph_id=gid,
                            node_type="task",
                            created_by_pid="p1",
                        ),
            ...
            assert list(store._projection_cache) == [
                (gid, version + 1)
            ]

    def test_schema_valid_cache_tamper_is_rejected_until_recovery(self, base_store):
        """A valid JSON edit must not be blessed as a durable revision.

        The commit fast path compares raw materialized rows with the
        hash-validated parent snapshot.  This adversarial case ensures that a
        schema-valid mutation (rather than malformed JSON) still fails closed,
        leaves the graph version untouched, and becomes writable only after an
        ...
        payload["title"] = "attacker-controlled-but-schema-valid"
        store.conn.execute(
            "UPDATE graph_nodes_projection SET payload_json = ? "
            "WHERE graph_id = ? AND node_id = ?",
            (json.dumps(payload), gid, "n1"),
        )
        store.conn.commit()
        # A raw cache mutation must invalidate the private validated-parent
        # entry before commit validation runs.
        assert store._cached_projection(gid, before) is None

        with pytest.raises(VPGError) as exc_info:
            rt.submit_patch(
                GraphPatchProposal(
                    graph_id=gid,
                    expected_graph_version=before,
                    author_pid="p1",
                    idempotency_key="tamper-rejected",
                    operations=(
                        AddNodeOp(
                            node_id="after-tamper",
                            graph_id=gid,
                            node_type="task",
                            created_by_pid="p1",
                        ),
        ...
        assert exc_info.value.code == VPGCode.GRAPH_RECOVERY_FAILED
        assert rt.get_graph(gid).current_version == before
        assert store.has_idempotency(("p1", gid, "tamper-rejected")) is None
        ...
        assert rt.get_graph(gid).current_version == before + 1
