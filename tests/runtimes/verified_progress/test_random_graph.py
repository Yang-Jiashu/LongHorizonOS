...
def _make_random_dag(seed: int, n_tasks: int, edge_prob: float):
    ...
    nodes = [AddNodeOp(node_id="g0", graph_id="_", node_type="goal",
                        created_by_pid="p1", title="Root")]
    for i in range(n_tasks):
        nodes.append(AddNodeOp(
            node_id=f"t{i}", graph_id="_", node_type="task",
            created_by_pid="p1", title=f"T{i}"))
    # goal -> first task always
    edges = [AddEdgeOp(
        edge_id="root_dep", edge_type="depends_on",
        ...
        for j in range(i + 1, n_tasks):
            if rng.random() < edge_prob:
                edges.append(AddEdgeOp(
                    edge_id=f"e_{i}_{j}", edge_type="depends_on",
                    source_node_id=f"t{j}", target_node_id=f"t{i}",
                    created_by_pid="p1"))
    return nodes, edges
...
class TestRandomGraphInvariants:
    ...
    def test_invariant_3_ready_frontier_subset_of_tasks(self, seed):
        ...
        tasks = {n.node_id for n in rt.store.get_all_nodes(gid)
    ...
    def test_invariant_8_partial_invalid_rollback(self):
        ...
        with pytest.raises(VPGError):
            _patch(rt, gid, "bad_and_good", (
                AddNodeOp(node_id="ok", graph_id=gid, node_type="task",
                           created_by_pid="p1", title="OK"),
                AddEdgeOp(edge_id="bad", edge_type="depends_on",
                           source_node_id="ghost", target_node_id="a",
                           created_by_pid="p1"),
            ))
...
def _run_battery():
    ...
    for trial, seed in enumerate([42, 7, 13, 99, 2024]):
        ...
        results.append({
            ...
            "task_count": sum(1 for n in rt.store.get_all_nodes(gid)
                              if n.node_type == NodeType.TASK),
