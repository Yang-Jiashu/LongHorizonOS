...
def test_async_agentos_benchmark_cli_json_is_machine_readable(capsys, benchmark_name):
    ...
    assert data["comparison"]["repetitions"] == 1
...
def test_online_compute_benchmark_cli_json_is_machine_readable(capsys):
    assert core.main(["benchmark", "online-compute", "--json"]) == 0
    data = json.loads(capsys.readouterr().out)
    assert data["benchmark"] == "longhorizonos-online-compute-control"
    assert data["valid"] is True
    assert data["comparison"]["token_reduction"] > 0
    assert data["scope"]["offline"] is True


def test_online_compute_benchmark_human_report_is_explicit_about_scope(capsys):
    assert core.main(["benchmark", "online-compute"]) == 0
    output = capsys.readouterr().out
    assert "ONLINE-COMPUTE CONTROL BENCHMARK" in output
    assert "deterministic offline simulator" in output
    assert "not LLM/GPU throughput" in output
...
def test_benchmark_help_promotes_async_agentos_name(capsys):
    ...
    assert "online-compute" in output
